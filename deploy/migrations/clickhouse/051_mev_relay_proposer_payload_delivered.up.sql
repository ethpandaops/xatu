CREATE TABLE default.mev_relay_proposer_payload_delivered_local ON CLUSTER '{cluster}' (
    ...
    `payload` String COMMENT 'The payload delivered' CODEC(ZSTD(1)),
    ...
    `timestamp` Int64 COMMENT 'The timestamp of the payload' CODEC(DoubleDelta, ZSTD(1)),
    `timestamp_ms` Int64 COMMENT 'The timestamp of the payload in milliseconds' CODEC(DoubleDelta, ZSTD(1)),
    `requested_at_slot_time` UInt64 COMMENT 'The time in the slot when the payload was requested' CODEC(ZSTD(1)),
    `response_at_slot_time` UInt64 COMMENT 'The time in the slot when the payload was delivered' CODEC(ZSTD(1)),
