CREATE TABLE default.node_record_consensus_local ON CLUSTER '{cluster}' (
    ...
    `enr_id` String COMMENT 'ID of the Ethereum Node Record' CODEC(ZSTD(1)),
    ...
    `csc` String COMMENT 'Client specific context' CODEC(ZSTD(1)),
    `finalized_start_date_time` Nullable(DateTime) COMMENT 'Finalized epoch start time' CODEC(DoubleDelta, ZSTD(1)),
    `head_start_date_time` Nullable(DateTime) COMMENT 'Head slot start time' CODEC(DoubleDelta, ZSTD(1)),
    `head_epoch_start_date_time` Nullable(DateTime) COMMENT 'Head epoch start time' CODEC(DoubleDelta, ZSTD(1)),
