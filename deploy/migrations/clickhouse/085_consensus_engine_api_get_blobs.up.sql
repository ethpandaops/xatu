CREATE TABLE consensus_engine_api_get_blobs_local ON CLUSTER '{cluster}' (
  -- Timestamps
...
ORDER BY (slot_start_date_time, meta_network_name, meta_client_name, block_root, event_date_time) COMMENT 'Contains timing and instrumentation data for engine_getBlobs calls between the consensus and execution layer.';

CREATE TABLE consensus_engine_api_get_blobs ON CLUSTER '{cluster}' AS consensus_engine_api_get_blobs_local
ENGINE = Distributed(
  '{cluster}',
  default,
  consensus_engine_api_get_blobs_local,
  cityHash64(
