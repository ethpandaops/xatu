...
ALTER TABLE canonical_execution_transaction_structlog_agg_local ON CLUSTER '{cluster}'
    ...
    ADD COLUMN IF NOT EXISTS `cold_access_count` UInt64 DEFAULT 0 COMMENT 'Number of cold storage/account accesses (EIP-2929). cold_gas = cold_count * (cold_cost - warm_cost).' CODEC(ZSTD(1)) AFTER `memory_words_sq_sum_after`;
