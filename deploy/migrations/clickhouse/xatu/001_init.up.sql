-- xatu raw-event schema (database-agnostic).
-- Applied per raw pipeline database via the migration matrix (e.g. default, devnets).
-- The target database is supplied at apply time (golang-migrate `database=`)
-- never hardcode a database name here. The {database} macro in Replicated paths
-- and currentDatabase() in Distributed engines keep this set portable across DBs.

CREATE TABLE IF NOT EXISTS beacon_api_eth_v1_beacon_blob_local ON CLUSTER '{cluster}'
(
...
CREATE TABLE IF NOT EXISTS beacon_api_eth_v1_beacon_committee_local ON CLUSTER '{cluster}'
(
...
CREATE TABLE IF NOT EXISTS beacon_api_eth_v1_events_attestation_local ON CLUSTER '{cluster}'
(
    `updated_date_time` DateTime COMMENT 'When this row was last updated' CODEC(DoubleDelta, ZSTD(1)),
    `event_date_time` DateTime64(3) COMMENT 'When the sentry received the event from a beacon node' CODEC(DoubleDelta, ZSTD(1)),
...
CREATE TABLE IF NOT EXISTS beacon_api_eth_v1_events_blob_sidecar_local ON CLUSTER '{cluster}'
(
    `updated_date_time` DateTime COMMENT 'Timestamp when the record was last updated' CODEC(DoubleDelta, ZSTD(1)),
    `event_date_time` DateTime64(3) COMMENT 'When the sentry received the event from a beacon node' CODEC(DoubleDelta, ZSTD(1)),
    `slot` UInt32 COMMENT 'Slot number in the beacon API event stream payload' CODEC(DoubleDelta, ZSTD(1)),
...
CREATE TABLE IF NOT EXISTS beacon_api_eth_v1_events_block_gossip_local ON CLUSTER '{cluster}'
(
    `updated_date_time` DateTime COMMENT 'Timestamp when the record was last updated' CODEC(DoubleDelta, ZSTD(1)),
    `event_date_time` DateTime64(3) COMMENT 'When the sentry received the event from a beacon node' CODEC(DoubleDelta, ZSTD(1)),
    `slot` UInt32 COMMENT 'Slot number in the beacon API event stream payload' CODEC(DoubleDelta, ZSTD(1)),
...
CREATE TABLE IF NOT EXISTS beacon_api_eth_v1_events_block_local ON CLUSTER '{cluster}'
(
    `updated_date_time` DateTime COMMENT 'Timestamp when the record was last updated' CODEC(DoubleDelta, ZSTD(1)),
    `event_date_time` DateTime64(3) COMMENT 'When the sentry received the event from a beacon node' CODEC(DoubleDelta, ZSTD(1)),
    `slot` UInt32 COMMENT 'Slot number in the beacon API event stream payload' CODEC(DoubleDelta, ZSTD(1)),
...
CREATE TABLE IF NOT EXISTS beacon_api_eth_v1_events_chain_reorg_local ON CLUSTER '{cluster}'
(
    `updated_date_time` DateTime COMMENT 'Timestamp when the record was last updated' CODEC(DoubleDelta, ZSTD(1)),
...
CREATE TABLE IF NOT EXISTS beacon_api_eth_v1_events_contribution_and_proof_local ON CLUSTER '{cluster}'
(
    `updated_date_time` DateTime COMMENT 'Timestamp when the record was last updated' CODEC(DoubleDelta, ZSTD(1)),
    `event_date_time` DateTime64(3) CODEC(DoubleDelta, ZSTD(1)),
...
CREATE TABLE IF NOT EXISTS beacon_api_eth_v1_events_data_column_sidecar_local ON CLUSTER '{cluster}'
(
    `updated_date_time` DateTime COMMENT 'Timestamp when the record was last updated' CODEC(DoubleDelta, ZSTD(1)),
    `event_date_time` DateTime64(3) COMMENT 'When the sentry received the event from a beacon node' CODEC(DoubleDelta, ZSTD(1)),
    `slot` UInt32 COMMENT 'Slot number in the beacon API event stream payload' CODEC(DoubleDelta, ZSTD(1)),
...
CREATE TABLE IF NOT EXISTS beacon_api_eth_v1_events_finalized_checkpoint_local ON CLUSTER '{cluster}'
(
    `updated_date_time` DateTime COMMENT 'Timestamp when the record was last updated' CODEC(DoubleDelta, ZSTD(1)),
...
CREATE TABLE IF NOT EXISTS beacon_api_eth_v1_events_head_local ON CLUSTER '{cluster}'
(
    `updated_date_time` DateTime COMMENT 'Timestamp when the record was last updated' CODEC(DoubleDelta, ZSTD(1)),
    `event_date_time` DateTime64(3) CODEC(DoubleDelta, ZSTD(1)),
...
CREATE TABLE IF NOT EXISTS beacon_api_eth_v1_events_voluntary_exit_local ON CLUSTER '{cluster}'
(
    `updated_date_time` DateTime COMMENT 'Timestamp when the record was last updated' CODEC(DoubleDelta, ZSTD(1)),
    `event_date_time` DateTime64(3) CODEC(DoubleDelta, ZSTD(1)),
    `epoch` UInt32 COMMENT 'The epoch number in the beacon API event stream payload',
...
CREATE TABLE IF NOT EXISTS beacon_api_eth_v1_proposer_duty_local ON CLUSTER '{cluster}'
(
    `updated_date_time` DateTime COMMENT 'When this row was last updated' CODEC(DoubleDelta, ZSTD(1)),
...
CREATE TABLE IF NOT EXISTS beacon_api_eth_v1_validator_attestation_data_local ON CLUSTER '{cluster}'
(
    `updated_date_time` DateTime COMMENT 'Timestamp when the record was last updated' CODEC(DoubleDelta, ZSTD(1)),
    `event_date_time` DateTime64(3) CODEC(DoubleDelta, ZSTD(1)),
...
CREATE TABLE IF NOT EXISTS beacon_api_eth_v2_beacon_block_local ON CLUSTER '{cluster}'
(
    `updated_date_time` DateTime COMMENT 'Timestamp when the record was last updated' CODEC(DoubleDelta, ZSTD(1)),
    `event_date_time` DateTime64(3) CODEC(DoubleDelta, ZSTD(1)),
...
CREATE TABLE IF NOT EXISTS beacon_api_eth_v3_validator_block_local ON CLUSTER '{cluster}'
(
    `updated_date_time` DateTime COMMENT 'Timestamp when the record was last updated' CODEC(DoubleDelta, ZSTD(1)),
    `event_date_time` DateTime64(3) COMMENT 'When the sentry received the event from a beacon node' CODEC(DoubleDelta, ZSTD(1)),
...
CREATE TABLE IF NOT EXISTS beacon_api_slot_local ON CLUSTER '{cluster}'
(
...
CREATE TABLE IF NOT EXISTS beacon_block_classification_local ON CLUSTER '{cluster}'
(
    `updated_date_time` DateTime COMMENT 'When this row was last updated' CODEC(DoubleDelta, ZSTD(1)),
...
CREATE TABLE IF NOT EXISTS blob_submitter_local ON CLUSTER '{cluster}'
(
    `updated_date_time` DateTime COMMENT 'Timestamp when the record was last updated' CODEC(DoubleDelta, ZSTD(1)),
...
CREATE TABLE IF NOT EXISTS block_native_mempool_transaction_local ON CLUSTER '{cluster}'
(
    `updated_date_time` DateTime COMMENT 'When this row was last updated' CODEC(DoubleDelta, ZSTD(1)),
...
CREATE TABLE IF NOT EXISTS canonical_beacon_blob_sidecar_local ON CLUSTER '{cluster}'
(
    `updated_date_time` DateTime COMMENT 'When this row was last updated' CODEC(DoubleDelta, ZSTD(1)),
    `slot` UInt32 COMMENT 'The slot number from beacon block payload' CODEC(DoubleDelta, ZSTD(1)),
    `slot_start_date_time` DateTime COMMENT 'The wall clock time when the slot started' CODEC(DoubleDelta, ZSTD(1)),
...
CREATE TABLE IF NOT EXISTS canonical_beacon_block_attester_slashing_local ON CLUSTER '{cluster}'
(
    `updated_date_time` DateTime COMMENT 'When this row was last updated' CODEC(DoubleDelta, ZSTD(1)),
    `slot` UInt32 COMMENT 'The slot number from beacon block payload' CODEC(DoubleDelta, ZSTD(1)),
    `slot_start_date_time` DateTime COMMENT 'The wall clock time when the slot started' CODEC(DoubleDelta, ZSTD(1)),
...
CREATE TABLE IF NOT EXISTS canonical_beacon_block_bls_to_execution_change_local ON CLUSTER '{cluster}'
(
    `updated_date_time` DateTime COMMENT 'When this row was last updated' CODEC(DoubleDelta, ZSTD(1)),
    `slot` UInt32 COMMENT 'The slot number from beacon block payload' CODEC(DoubleDelta, ZSTD(1)),
    `slot_start_date_time` DateTime COMMENT 'The wall clock time when the slot started' CODEC(DoubleDelta, ZSTD(1)),
...
CREATE TABLE IF NOT EXISTS canonical_beacon_block_deposit_local ON CLUSTER '{cluster}'
(
    `updated_date_time` DateTime COMMENT 'When this row was last updated' CODEC(DoubleDelta, ZSTD(1)),
    `slot` UInt32 COMMENT 'The slot number from beacon block payload' CODEC(DoubleDelta, ZSTD(1)),
    `slot_start_date_time` DateTime COMMENT 'The wall clock time when the slot started' CODEC(DoubleDelta, ZSTD(1)),
...
CREATE TABLE IF NOT EXISTS canonical_beacon_block_execution_transaction_local ON CLUSTER '{cluster}'
(
    `updated_date_time` DateTime COMMENT 'When this row was last updated' CODEC(DoubleDelta, ZSTD(1)),
    `slot` UInt32 COMMENT 'The slot number from beacon block payload' CODEC(DoubleDelta, ZSTD(1)),
    `slot_start_date_time` DateTime COMMENT 'The wall clock time when the slot started' CODEC(DoubleDelta, ZSTD(1)),
...
CREATE TABLE IF NOT EXISTS canonical_beacon_block_local ON CLUSTER '{cluster}'
(
    `updated_date_time` DateTime COMMENT 'When this row was last updated' CODEC(DoubleDelta, ZSTD(1)),
    `slot` UInt32 COMMENT 'The slot number from beacon block payload' CODEC(DoubleDelta, ZSTD(1)),
    `slot_start_date_time` DateTime COMMENT 'The wall clock time when the slot started' CODEC(DoubleDelta, ZSTD(1)),
...
CREATE TABLE IF NOT EXISTS canonical_beacon_block_proposer_slashing_local ON CLUSTER '{cluster}'
(
    `updated_date_time` DateTime COMMENT 'When this row was last updated' CODEC(DoubleDelta, ZSTD(1)),
    `slot` UInt32 COMMENT 'The slot number from beacon block payload' CODEC(DoubleDelta, ZSTD(1)),
    `slot_start_date_time` DateTime COMMENT 'The wall clock time when the slot started' CODEC(DoubleDelta, ZSTD(1)),
...
CREATE TABLE IF NOT EXISTS canonical_beacon_block_sync_aggregate_local ON CLUSTER '{cluster}'
(
    `updated_date_time` DateTime COMMENT 'Timestamp when the record was last updated' CODEC(DoubleDelta, ZSTD(1)),
    `event_date_time` DateTime64(3) COMMENT 'When the sentry received the event from a beacon node' CODEC(DoubleDelta, ZSTD(1)),
...
CREATE TABLE IF NOT EXISTS canonical_beacon_block_voluntary_exit_local ON CLUSTER '{cluster}'
(
    `updated_date_time` DateTime COMMENT 'When this row was last updated' CODEC(DoubleDelta, ZSTD(1)),
    `slot` UInt32 COMMENT 'The slot number from beacon block payload' CODEC(DoubleDelta, ZSTD(1)),
    `slot_start_date_time` DateTime COMMENT 'The wall clock time when the slot started' CODEC(DoubleDelta, ZSTD(1)),
...
CREATE TABLE IF NOT EXISTS canonical_beacon_block_withdrawal_local ON CLUSTER '{cluster}'
(
    `updated_date_time` DateTime COMMENT 'When this row was last updated' CODEC(DoubleDelta, ZSTD(1)),
    `slot` UInt32 COMMENT 'The slot number from beacon block payload' CODEC(DoubleDelta, ZSTD(1)),
    `slot_start_date_time` DateTime COMMENT 'The wall clock time when the slot started' CODEC(DoubleDelta, ZSTD(1)),
...
CREATE TABLE IF NOT EXISTS canonical_beacon_committee_local ON CLUSTER '{cluster}'
(
    `updated_date_time` DateTime COMMENT 'Timestamp when the record was last updated' CODEC(DoubleDelta, ZSTD(1)),
    `slot` UInt32 COMMENT 'Slot number in the beacon API committee payload' CODEC(DoubleDelta, ZSTD(1)),
    `slot_start_date_time` DateTime COMMENT 'The wall clock time when the slot started' CODEC(DoubleDelta, ZSTD(1)),
...
CREATE TABLE IF NOT EXISTS canonical_beacon_elaborated_attestation_local ON CLUSTER '{cluster}'
(
    `updated_date_time` DateTime COMMENT 'When this row was last updated' CODEC(DoubleDelta, ZSTD(1)),
...
CREATE TABLE IF NOT EXISTS canonical_beacon_proposer_duty_local ON CLUSTER '{cluster}'
(
    `updated_date_time` DateTime COMMENT 'When this row was last updated' CODEC(DoubleDelta, ZSTD(1)),
...
CREATE TABLE IF NOT EXISTS canonical_beacon_sync_committee_local ON CLUSTER '{cluster}'
(
    `updated_date_time` DateTime COMMENT 'Timestamp when the record was last updated' CODEC(DoubleDelta, ZSTD(1)),
    `event_date_time` DateTime64(3) COMMENT 'When the sentry received the event from a beacon node' CODEC(DoubleDelta, ZSTD(1)),
...
CREATE TABLE IF NOT EXISTS canonical_beacon_validators_local ON CLUSTER '{cluster}'
(
    `updated_date_time` DateTime COMMENT 'When this row was last updated' CODEC(DoubleDelta, ZSTD(1)),
    `epoch` UInt32 COMMENT 'The epoch number from beacon block payload' CODEC(DoubleDelta, ZSTD(1)),
    `epoch_start_date_time` DateTime COMMENT 'The wall clock time when the epoch started' CODEC(DoubleDelta, ZSTD(1)),
...
CREATE TABLE IF NOT EXISTS canonical_beacon_validators_pubkeys_local ON CLUSTER '{cluster}'
(
    `updated_date_time` DateTime COMMENT 'When this row was last updated' CODEC(DoubleDelta, ZSTD(1)),
    `epoch` UInt32 COMMENT 'The epoch number from beacon block payload' CODEC(DoubleDelta, ZSTD(1)),
    `epoch_start_date_time` DateTime COMMENT 'The wall clock time when the epoch started' CODEC(DoubleDelta, ZSTD(1)),
...
CREATE TABLE IF NOT EXISTS canonical_beacon_validators_withdrawal_credentials_local ON CLUSTER '{cluster}'
(
    `updated_date_time` DateTime COMMENT 'When this row was last updated' CODEC(DoubleDelta, ZSTD(1)),
    `epoch` UInt32 COMMENT 'The epoch number from beacon block payload' CODEC(DoubleDelta, ZSTD(1)),
    `epoch_start_date_time` DateTime COMMENT 'The wall clock time when the epoch started' CODEC(DoubleDelta, ZSTD(1)),
...
CREATE TABLE IF NOT EXISTS canonical_execution_address_appearances_local ON CLUSTER '{cluster}'
(
    `updated_date_time` DateTime COMMENT 'Timestamp when the record was last updated' CODEC(DoubleDelta, ZSTD(1)),
...
CREATE TABLE IF NOT EXISTS canonical_execution_balance_diffs_local ON CLUSTER '{cluster}'
(
    `updated_date_time` DateTime COMMENT 'Timestamp when the record was last updated' CODEC(DoubleDelta, ZSTD(1)),
    `block_number` UInt64 COMMENT 'The block number' CODEC(DoubleDelta, ZSTD(1)),
...
CREATE TABLE IF NOT EXISTS canonical_execution_balance_reads_local ON CLUSTER '{cluster}'
(
    `updated_date_time` DateTime COMMENT 'Timestamp when the record was last updated' CODEC(DoubleDelta, ZSTD(1)),
    `block_number` UInt64 COMMENT 'The block number' CODEC(DoubleDelta, ZSTD(1)),
    `transaction_index` UInt64 COMMENT 'The transaction index in the block' CODEC(DoubleDelta, ZSTD(1)),
...
CREATE TABLE IF NOT EXISTS canonical_execution_block_local ON CLUSTER '{cluster}'
(
    `updated_date_time` DateTime COMMENT 'Timestamp when the record was last updated' CODEC(DoubleDelta, ZSTD(1)),
...
CREATE TABLE IF NOT EXISTS canonical_execution_contracts_local ON CLUSTER '{cluster}'
(
    `updated_date_time` DateTime COMMENT 'Timestamp when the record was last updated' CODEC(DoubleDelta, ZSTD(1)),
    `block_number` UInt64 COMMENT 'The block number' CODEC(DoubleDelta, ZSTD(1)),
...
CREATE TABLE IF NOT EXISTS canonical_execution_erc20_transfers_local ON CLUSTER '{cluster}'
(
    `updated_date_time` DateTime COMMENT 'Timestamp when the record was last updated' CODEC(DoubleDelta, ZSTD(1)),
    `block_number` UInt64 COMMENT 'The block number' CODEC(DoubleDelta, ZSTD(1)),
    `transaction_index` UInt64 COMMENT 'The transaction index in the block' CODEC(DoubleDelta, ZSTD(1)),
...
CREATE TABLE IF NOT EXISTS canonical_execution_erc721_transfers_local ON CLUSTER '{cluster}'
(
    `updated_date_time` DateTime COMMENT 'Timestamp when the record was last updated' CODEC(DoubleDelta, ZSTD(1)),
    `block_number` UInt64 COMMENT 'The block number' CODEC(DoubleDelta, ZSTD(1)),
    `transaction_index` UInt64 COMMENT 'The transaction index in the block' CODEC(DoubleDelta, ZSTD(1)),
...
CREATE TABLE IF NOT EXISTS canonical_execution_four_byte_counts_local ON CLUSTER '{cluster}'
(
    `updated_date_time` DateTime COMMENT 'Timestamp when the record was last updated' CODEC(DoubleDelta, ZSTD(1)),
    `block_number` UInt64 COMMENT 'The block number' CODEC(DoubleDelta, ZSTD(1)),
    `transaction_index` UInt64 COMMENT 'The transaction index in the block' CODEC(DoubleDelta, ZSTD(1)),
...
CREATE TABLE IF NOT EXISTS canonical_execution_native_transfers_local ON CLUSTER '{cluster}'
(
    `updated_date_time` DateTime COMMENT 'Timestamp when the record was last updated' CODEC(DoubleDelta, ZSTD(1)),
    `block_number` UInt64 COMMENT 'The block number' CODEC(DoubleDelta, ZSTD(1)),
    `transaction_index` UInt64 COMMENT 'The transaction index in the block' CODEC(DoubleDelta, ZSTD(1)),
...
CREATE TABLE IF NOT EXISTS canonical_execution_nonce_diffs_local ON CLUSTER '{cluster}'
(
    `updated_date_time` DateTime COMMENT 'Timestamp when the record was last updated' CODEC(DoubleDelta, ZSTD(1)),
    `block_number` UInt64 COMMENT 'The block number' CODEC(DoubleDelta, ZSTD(1)),
    `transaction_index` UInt64 COMMENT 'The transaction index in the block' CODEC(DoubleDelta, ZSTD(1)),
...
CREATE TABLE IF NOT EXISTS canonical_execution_nonce_reads_local ON CLUSTER '{cluster}'
(
    `updated_date_time` DateTime COMMENT 'Timestamp when the record was last updated' CODEC(DoubleDelta, ZSTD(1)),
    `block_number` UInt64 COMMENT 'The block number' CODEC(DoubleDelta, ZSTD(1)),
    `transaction_index` UInt64 COMMENT 'The transaction index in the block' CODEC(DoubleDelta, ZSTD(1)),
...
CREATE TABLE IF NOT EXISTS canonical_execution_storage_diffs_local ON CLUSTER '{cluster}'
(
    `updated_date_time` DateTime COMMENT 'Timestamp when the record was last updated' CODEC(DoubleDelta, ZSTD(1)),
    `block_number` UInt64 COMMENT 'The block number' CODEC(DoubleDelta, ZSTD(1)),
    `transaction_index` UInt64 COMMENT 'The transaction index' CODEC(DoubleDelta, ZSTD(1)),
...
CREATE TABLE IF NOT EXISTS canonical_execution_storage_reads_local ON CLUSTER '{cluster}'
(
    `updated_date_time` DateTime COMMENT 'Timestamp when the record was last updated' CODEC(DoubleDelta, ZSTD(1)),
    `block_number` UInt64 COMMENT 'The block number' CODEC(DoubleDelta, ZSTD(1)),
    `transaction_index` UInt64 COMMENT 'The transaction index' CODEC(DoubleDelta, ZSTD(1)),
...
CREATE TABLE IF NOT EXISTS canonical_execution_traces_local ON CLUSTER '{cluster}'
(
    `updated_date_time` DateTime COMMENT 'Timestamp when the record was last updated' CODEC(DoubleDelta, ZSTD(1)),
    `block_number` UInt64 COMMENT 'The block number' CODEC(DoubleDelta, ZSTD(1)),
    `transaction_index` UInt64 COMMENT 'The transaction index' CODEC(DoubleDelta, ZSTD(1)),
...
CREATE TABLE IF NOT EXISTS canonical_execution_transaction_local ON CLUSTER '{cluster}'
(
    `updated_date_time` DateTime COMMENT 'Timestamp when the record was last updated' CODEC(DoubleDelta, ZSTD(1)),
    `block_number` UInt64 COMMENT 'The block number' CODEC(DoubleDelta, ZSTD(1)),
    `transaction_index` UInt64 COMMENT 'The transaction index' CODEC(DoubleDelta, ZSTD(1)),
...
CREATE TABLE IF NOT EXISTS canonical_execution_transaction_structlog_agg_local ON CLUSTER '{cluster}'
(
    `updated_date_time` DateTime COMMENT 'Timestamp when the record was last updated' CODEC(DoubleDelta, ZSTD(1)),
    `block_number` UInt64 COMMENT 'The block number' CODEC(DoubleDelta, ZSTD(1)),
    `transaction_hash` FixedString(66) COMMENT 'The transaction hash' CODEC(ZSTD(1)),
...
CREATE TABLE IF NOT EXISTS canonical_execution_transaction_structlog_local ON CLUSTER '{cluster}'
(
    `updated_date_time` DateTime COMMENT 'Timestamp when the record was last updated' CODEC(DoubleDelta, ZSTD(1)),
    `block_number` UInt64 COMMENT 'The block number' CODEC(DoubleDelta, ZSTD(1)),
    `transaction_hash` FixedString(66) COMMENT 'The transaction hash' CODEC(ZSTD(1)),
...
CREATE TABLE IF NOT EXISTS consensus_engine_api_get_blobs_local ON CLUSTER '{cluster}'
(
    `updated_date_time` DateTime COMMENT 'Timestamp when the record was last updated' CODEC(DoubleDelta, ZSTD(1)),
...
CREATE TABLE IF NOT EXISTS consensus_engine_api_new_payload_local ON CLUSTER '{cluster}'
(
    `updated_date_time` DateTime COMMENT 'Timestamp when the record was last updated' CODEC(DoubleDelta, ZSTD(1)),
    `event_date_time` DateTime64(3) COMMENT 'When the sentry received the event' CODEC(DoubleDelta, ZSTD(1)),
...
CREATE TABLE IF NOT EXISTS ethseer_validator_entity_local ON CLUSTER '{cluster}'
(
    `updated_date_time` DateTime COMMENT 'Timestamp when the record was last updated' CODEC(DoubleDelta, ZSTD(1)),
...
CREATE TABLE IF NOT EXISTS execution_block_metrics_local ON CLUSTER '{cluster}'
(
    `updated_date_time` DateTime COMMENT 'Timestamp when the record was last updated' CODEC(DoubleDelta, ZSTD(1)),
...
CREATE TABLE IF NOT EXISTS execution_engine_get_blobs_local ON CLUSTER '{cluster}'
(
    `updated_date_time` DateTime COMMENT 'Timestamp when the record was last updated' CODEC(DoubleDelta, ZSTD(1)),
    `event_date_time` DateTime64(3) COMMENT 'When the event was received' CODEC(DoubleDelta, ZSTD(1)),
...
CREATE TABLE IF NOT EXISTS execution_engine_new_payload_local ON CLUSTER '{cluster}'
(
    `updated_date_time` DateTime COMMENT 'Timestamp when the record was last updated' CODEC(DoubleDelta, ZSTD(1)),
    `event_date_time` DateTime64(3) COMMENT 'When the event was received' CODEC(DoubleDelta, ZSTD(1)),
...
CREATE TABLE IF NOT EXISTS execution_state_size_local ON CLUSTER '{cluster}'
(
    `updated_date_time` DateTime COMMENT 'Timestamp when the record was last updated' CODEC(DoubleDelta, ZSTD(1)),
...
CREATE TABLE IF NOT EXISTS execution_transaction_local ON CLUSTER '{cluster}'
(
    `updated_date_time` DateTime COMMENT 'Timestamp when the record was last updated' CODEC(DoubleDelta, ZSTD(1)),
    `block_number` UInt64 COMMENT 'The block number' CODEC(DoubleDelta, ZSTD(1)),
    `block_hash` FixedString(66) COMMENT 'The block hash' CODEC(ZSTD(1)),
...
CREATE TABLE IF NOT EXISTS imported_sources_local ON CLUSTER '{cluster}'
(
...
CREATE TABLE IF NOT EXISTS libp2p_add_peer_local ON CLUSTER '{cluster}'
(
    `updated_date_time` DateTime COMMENT 'Timestamp when the record was last updated' CODEC(DoubleDelta, ZSTD(1)),
...
CREATE TABLE IF NOT EXISTS libp2p_connected_local ON CLUSTER '{cluster}'
(
    `updated_date_time` DateTime COMMENT 'Timestamp when the record was last updated' CODEC(DoubleDelta, ZSTD(1)),
    `event_date_time` DateTime64(3) COMMENT 'Timestamp of the event' CODEC(DoubleDelta, ZSTD(1)),
...
CREATE TABLE IF NOT EXISTS libp2p_deliver_message_local ON CLUSTER '{cluster}'
(
    `updated_date_time` DateTime COMMENT 'Timestamp when the record was last updated' CODEC(DoubleDelta, ZSTD(1)),
    `event_date_time` DateTime64(3) COMMENT 'Timestamp of the event' CODEC(DoubleDelta, ZSTD(1)),
...
CREATE TABLE IF NOT EXISTS libp2p_disconnected_local ON CLUSTER '{cluster}'
(
    `updated_date_time` DateTime COMMENT 'Timestamp when the record was last updated' CODEC(DoubleDelta, ZSTD(1)),
    `event_date_time` DateTime64(3) COMMENT 'Timestamp of the event' CODEC(DoubleDelta, ZSTD(1)),
    `remote_peer_id_unique_key` Int64 COMMENT 'Unique key associated with the identifier of the remote peer',
...
CREATE TABLE IF NOT EXISTS libp2p_drop_rpc_local ON CLUSTER '{cluster}'
(
...
CREATE TABLE IF NOT EXISTS libp2p_duplicate_message_local ON CLUSTER '{cluster}'
(
    `updated_date_time` DateTime COMMENT 'Timestamp when the record was last updated' CODEC(DoubleDelta, ZSTD(1)),
    `event_date_time` DateTime64(3) COMMENT 'Timestamp of the event' CODEC(DoubleDelta, ZSTD(1)),
    `topic_layer` LowCardinality(String) COMMENT 'Layer of the topic',
...
CREATE TABLE IF NOT EXISTS libp2p_gossipsub_aggregate_and_proof_local ON CLUSTER '{cluster}'
(
    `updated_date_time` DateTime COMMENT 'Timestamp when the record was last updated' CODEC(DoubleDelta, ZSTD(1)),
...
CREATE TABLE IF NOT EXISTS libp2p_gossipsub_beacon_attestation_local ON CLUSTER '{cluster}'
(
    `updated_date_time` DateTime COMMENT 'Timestamp when the record was last updated' CODEC(DoubleDelta, ZSTD(1)),
    `event_date_time` DateTime64(3) COMMENT 'Timestamp of the event with millisecond precision' CODEC(DoubleDelta, ZSTD(1)),
    `slot` UInt32 COMMENT 'Slot number associated with the event' CODEC(DoubleDelta, ZSTD(1)),
...
CREATE TABLE IF NOT EXISTS libp2p_gossipsub_beacon_block_local ON CLUSTER '{cluster}'
(
    `updated_date_time` DateTime COMMENT 'Timestamp when the record was last updated' CODEC(DoubleDelta, ZSTD(1)),
    `event_date_time` DateTime64(3) COMMENT 'Timestamp of the event with millisecond precision' CODEC(DoubleDelta, ZSTD(1)),
    `slot` UInt32 COMMENT 'Slot number associated with the event' CODEC(DoubleDelta, ZSTD(1)),
...
CREATE TABLE IF NOT EXISTS libp2p_gossipsub_blob_sidecar_local ON CLUSTER '{cluster}'
(
    `updated_date_time` DateTime COMMENT 'Timestamp when the record was last updated' CODEC(DoubleDelta, ZSTD(1)),
    `event_date_time` DateTime64(3) COMMENT 'Timestamp of the event with millisecond precision' CODEC(DoubleDelta, ZSTD(1)),
    `slot` UInt32 COMMENT 'Slot number associated with the event' CODEC(DoubleDelta, ZSTD(1)),
...
CREATE TABLE IF NOT EXISTS libp2p_gossipsub_data_column_sidecar_local ON CLUSTER '{cluster}'
(
    `updated_date_time` DateTime COMMENT 'Timestamp when the record was last updated' CODEC(DoubleDelta, ZSTD(1)),
    `event_date_time` DateTime64(3) COMMENT 'Timestamp of the event with millisecond precision' CODEC(DoubleDelta, ZSTD(1)),
    `slot` UInt32 COMMENT 'Slot number associated with the event' CODEC(DoubleDelta, ZSTD(1)),
...
CREATE TABLE IF NOT EXISTS libp2p_graft_local ON CLUSTER '{cluster}'
(
    `updated_date_time` DateTime COMMENT 'Timestamp when the record was last updated' CODEC(DoubleDelta, ZSTD(1)),
    `event_date_time` DateTime64(3) COMMENT 'Timestamp of the event' CODEC(DoubleDelta, ZSTD(1)),
    `topic_layer` LowCardinality(String) COMMENT 'Layer of the topic',
...
CREATE TABLE IF NOT EXISTS libp2p_handle_metadata_local ON CLUSTER '{cluster}'
(
    `updated_date_time` DateTime COMMENT 'Timestamp when the record was last updated' CODEC(DoubleDelta, ZSTD(1)),
    `event_date_time` DateTime64(3) COMMENT 'Timestamp of the event' CODEC(DoubleDelta, ZSTD(1)),
...
CREATE TABLE IF NOT EXISTS libp2p_handle_status_local ON CLUSTER '{cluster}'
(
    `updated_date_time` DateTime COMMENT 'Timestamp when the record was last updated' CODEC(DoubleDelta, ZSTD(1)),
    `event_date_time` DateTime64(3) COMMENT 'Timestamp of the event' CODEC(DoubleDelta, ZSTD(1)),
    `peer_id_unique_key` Int64 COMMENT 'Unique key associated with the identifier of the peer',
...
CREATE TABLE IF NOT EXISTS libp2p_peer_local ON CLUSTER '{cluster}'
(
...
CREATE TABLE IF NOT EXISTS libp2p_prune_local ON CLUSTER '{cluster}'
(
    `updated_date_time` DateTime COMMENT 'Timestamp when the record was last updated' CODEC(DoubleDelta, ZSTD(1)),
    `event_date_time` DateTime64(3) COMMENT 'Timestamp of the event' CODEC(DoubleDelta, ZSTD(1)),
    `topic_layer` LowCardinality(String) COMMENT 'Layer of the topic',
...
CREATE TABLE IF NOT EXISTS libp2p_publish_message_local ON CLUSTER '{cluster}'
(
    `updated_date_time` DateTime COMMENT 'Timestamp when the record was last updated' CODEC(DoubleDelta, ZSTD(1)),
    `event_date_time` DateTime64(3) COMMENT 'Timestamp of the event' CODEC(DoubleDelta, ZSTD(1)),
    `topic_layer` LowCardinality(String) COMMENT 'Layer of the topic',
...
CREATE TABLE IF NOT EXISTS libp2p_recv_rpc_local ON CLUSTER '{cluster}'
(
    `unique_key` Int64 COMMENT 'Unique identifier for each record',
    `updated_date_time` DateTime COMMENT 'Timestamp when the record was last updated' CODEC(DoubleDelta, ZSTD(1)),
    `event_date_time` DateTime64(3) COMMENT 'Timestamp of the event' CODEC(DoubleDelta, ZSTD(1)),
...
CREATE TABLE IF NOT EXISTS libp2p_reject_message_local ON CLUSTER '{cluster}'
(
    `updated_date_time` DateTime COMMENT 'Timestamp when the record was last updated' CODEC(DoubleDelta, ZSTD(1)),
    `event_date_time` DateTime64(3) COMMENT 'Timestamp of the event' CODEC(DoubleDelta, ZSTD(1)),
    `topic_layer` LowCardinality(String) COMMENT 'Layer of the topic',
...
CREATE TABLE IF NOT EXISTS libp2p_remove_peer_local ON CLUSTER '{cluster}'
(
    `updated_date_time` DateTime COMMENT 'Timestamp when the record was last updated' CODEC(DoubleDelta, ZSTD(1)),
    `event_date_time` DateTime64(3) COMMENT 'Timestamp of the event' CODEC(DoubleDelta, ZSTD(1)),
    `peer_id_unique_key` Int64 COMMENT 'Unique key associated with the identifier of the peer',
...
CREATE TABLE IF NOT EXISTS libp2p_rpc_meta_control_graft_local ON CLUSTER '{cluster}'
(
...
CREATE TABLE IF NOT EXISTS libp2p_rpc_meta_control_idontwant_local ON CLUSTER '{cluster}'
(
...
CREATE TABLE IF NOT EXISTS libp2p_rpc_meta_control_ihave_local ON CLUSTER '{cluster}'
(
...
CREATE TABLE IF NOT EXISTS libp2p_rpc_meta_control_iwant_local ON CLUSTER '{cluster}'
(
...
CREATE TABLE IF NOT EXISTS libp2p_rpc_meta_control_prune_local ON CLUSTER '{cluster}'
(
...
CREATE TABLE IF NOT EXISTS libp2p_rpc_meta_message_local ON CLUSTER '{cluster}'
(
...
CREATE TABLE IF NOT EXISTS libp2p_rpc_meta_subscription_local ON CLUSTER '{cluster}'
(
...
CREATE TABLE IF NOT EXISTS libp2p_send_rpc_local ON CLUSTER '{cluster}'
(
    `unique_key` Int64 COMMENT 'Unique identifier for each record',
    `updated_date_time` DateTime COMMENT 'Timestamp when the record was last updated' CODEC(DoubleDelta, ZSTD(1)),
    `event_date_time` DateTime64(3) COMMENT 'Timestamp of the event' CODEC(DoubleDelta, ZSTD(1)),
...
CREATE TABLE IF NOT EXISTS libp2p_synthetic_heartbeat_local ON CLUSTER '{cluster}'
(
    `updated_date_time` DateTime COMMENT 'Timestamp when the record was last updated' CODEC(DoubleDelta, ZSTD(1)),
...
CREATE TABLE IF NOT EXISTS mempool_dumpster_transaction_local ON CLUSTER '{cluster}'
(
...
CREATE TABLE IF NOT EXISTS mempool_transaction_local ON CLUSTER '{cluster}'
(
    `updated_date_time` DateTime COMMENT 'Timestamp when the record was last updated' CODEC(DoubleDelta, ZSTD(1)),
...
CREATE TABLE IF NOT EXISTS mev_relay_bid_trace_local ON CLUSTER '{cluster}'
(
    `updated_date_time` DateTime COMMENT 'Timestamp when the record was last updated' CODEC(DoubleDelta, ZSTD(1)),
...
CREATE TABLE IF NOT EXISTS mev_relay_proposer_payload_delivered_local ON CLUSTER '{cluster}'
(
    `updated_date_time` DateTime COMMENT 'Timestamp when the record was last updated' CODEC(DoubleDelta, ZSTD(1)),
...
CREATE TABLE IF NOT EXISTS node_record_consensus_local ON CLUSTER '{cluster}'
(
    `updated_date_time` DateTime COMMENT 'Timestamp when the record was last updated' CODEC(DoubleDelta, ZSTD(1)),
...
CREATE TABLE IF NOT EXISTS node_record_execution_local ON CLUSTER '{cluster}'
(
    `updated_date_time` DateTime COMMENT 'Timestamp when the record was last updated' CODEC(DoubleDelta, ZSTD(1)),
...
CREATE TABLE IF NOT EXISTS beacon_api_eth_v1_beacon_blob ON CLUSTER '{cluster}'
AS beacon_api_eth_v1_beacon_blob_local
ENGINE = Distributed('{cluster}', currentDatabase(), 'beacon_api_eth_v1_beacon_blob_local', cityHash64(slot_start_date_time, meta_client_name, block_root))
COMMENT 'Contains beacon API blob metadata derived from block blob_kzg_commitments from each sentry client attached to a beacon node.';

CREATE TABLE IF NOT EXISTS beacon_api_eth_v1_beacon_committee ON CLUSTER '{cluster}'
AS beacon_api_eth_v1_beacon_committee_local
ENGINE = Distributed('{cluster}', currentDatabase(), 'beacon_api_eth_v1_beacon_committee_local', cityHash64(slot_start_date_time, meta_network_name, meta_client_name, committee_index))
COMMENT 'Contains beacon API /eth/v1/beacon/states/{state_id}/committees data from each sentry client attached to a beacon node.';

CREATE TABLE IF NOT EXISTS beacon_api_eth_v1_events_attestation ON CLUSTER '{cluster}'
AS beacon_api_eth_v1_events_attestation_local
ENGINE = Distributed('{cluster}', currentDatabase(), 'beacon_api_eth_v1_events_attestation_local', cityHash64(slot_start_date_time, meta_client_name))
COMMENT 'Contains beacon API eventstream "attestation" data from each sentry client attached to a beacon node.';

CREATE TABLE IF NOT EXISTS beacon_api_eth_v1_events_blob_sidecar ON CLUSTER '{cluster}'
AS beacon_api_eth_v1_events_blob_sidecar_local
ENGINE = Distributed('{cluster}', currentDatabase(), 'beacon_api_eth_v1_events_blob_sidecar_local', cityHash64(slot_start_date_time, meta_network_name, meta_client_name, block_root, blob_index))
COMMENT 'Contains beacon API eventstream "blob_sidecar" data from each sentry client attached to a beacon node.';

CREATE TABLE IF NOT EXISTS beacon_api_eth_v1_events_block ON CLUSTER '{cluster}'
AS beacon_api_eth_v1_events_block_local
ENGINE = Distributed('{cluster}', currentDatabase(), 'beacon_api_eth_v1_events_block_local', cityHash64(slot_start_date_time, meta_network_name, meta_client_name, block))
COMMENT 'Contains beacon API eventstream "block" data from each sentry client attached to a beacon node.';

CREATE TABLE IF NOT EXISTS beacon_api_eth_v1_events_block_gossip ON CLUSTER '{cluster}'
AS beacon_api_eth_v1_events_block_gossip_local
ENGINE = Distributed('{cluster}', currentDatabase(), 'beacon_api_eth_v1_events_block_gossip_local', cityHash64(slot_start_date_time, meta_network_name, meta_client_name, block))
COMMENT 'Contains beacon API eventstream "block_gossip" data from each sentry client attached to a beacon node.';

CREATE TABLE IF NOT EXISTS beacon_api_eth_v1_events_chain_reorg ON CLUSTER '{cluster}'
AS beacon_api_eth_v1_events_chain_reorg_local
ENGINE = Distributed('{cluster}', currentDatabase(), 'beacon_api_eth_v1_events_chain_reorg_local', cityHash64(slot_start_date_time, meta_network_name, meta_client_name, old_head_block, new_head_block))
COMMENT 'Contains beacon API eventstream "chain reorg" data from each sentry client attached to a beacon node.';

CREATE TABLE IF NOT EXISTS beacon_api_eth_v1_events_contribution_and_proof ON CLUSTER '{cluster}'
AS beacon_api_eth_v1_events_contribution_and_proof_local
ENGINE = Distributed('{cluster}', currentDatabase(), 'beacon_api_eth_v1_events_contribution_and_proof_local', cityHash64(contribution_slot_start_date_time, meta_network_name, meta_client_name, contribution_beacon_block_root, contribution_subcommittee_index, signature))
COMMENT 'Contains beacon API eventstream "contribution and proof" data from each sentry client attached to a beacon node.';

CREATE TABLE IF NOT EXISTS beacon_api_eth_v1_events_data_column_sidecar ON CLUSTER '{cluster}'
AS beacon_api_eth_v1_events_data_column_sidecar_local
ENGINE = Distributed('{cluster}', currentDatabase(), 'beacon_api_eth_v1_events_data_column_sidecar_local', cityHash64(slot_start_date_time, meta_network_name, meta_client_name, block_root, column_index))
COMMENT 'Contains beacon API eventstream "data_column_sidecar" data from each sentry client attached to a beacon node.';

CREATE TABLE IF NOT EXISTS beacon_api_eth_v1_events_finalized_checkpoint ON CLUSTER '{cluster}'
AS beacon_api_eth_v1_events_finalized_checkpoint_local
ENGINE = Distributed('{cluster}', currentDatabase(), 'beacon_api_eth_v1_events_finalized_checkpoint_local', cityHash64(epoch_start_date_time, meta_network_name, meta_client_name, block, state))
COMMENT 'Contains beacon API eventstream "finalized checkpoint" data from each sentry client attached to a beacon node.';

CREATE TABLE IF NOT EXISTS beacon_api_eth_v1_events_head ON CLUSTER '{cluster}'
AS beacon_api_eth_v1_events_head_local
ENGINE = Distributed('{cluster}', currentDatabase(), 'beacon_api_eth_v1_events_head_local', cityHash64(slot_start_date_time, meta_network_name, meta_client_name, block, previous_duty_dependent_root, current_duty_dependent_root))
...
CREATE TABLE IF NOT EXISTS beacon_api_eth_v1_events_voluntary_exit ON CLUSTER '{cluster}'
AS beacon_api_eth_v1_events_voluntary_exit_local
ENGINE = Distributed('{cluster}', currentDatabase(), 'beacon_api_eth_v1_events_voluntary_exit_local', cityHash64(wallclock_epoch_start_date_time, meta_network_name, meta_client_name, validator_index))
COMMENT 'Contains beacon API eventstream "voluntary exit" data from each sentry client attached to a beacon node.';

CREATE TABLE IF NOT EXISTS beacon_api_eth_v1_proposer_duty ON CLUSTER '{cluster}'
AS beacon_api_eth_v1_proposer_duty_local
ENGINE = Distributed('{cluster}', currentDatabase(), 'beacon_api_eth_v1_proposer_duty_local', cityHash64(slot_start_date_time, meta_network_name, meta_client_name, proposer_validator_index))
COMMENT 'Contains a proposer duty from a beacon block.';

CREATE TABLE IF NOT EXISTS beacon_api_eth_v1_validator_attestation_data ON CLUSTER '{cluster}'
AS beacon_api_eth_v1_validator_attestation_data_local
ENGINE = Distributed('{cluster}', currentDatabase(), 'beacon_api_eth_v1_validator_attestation_data_local', cityHash64(slot_start_date_time, meta_network_name, meta_client_name, committee_index, beacon_block_root, source_root, target_root))
COMMENT 'Contains beacon API validator attestation data from each sentry client attached to a beacon node.';

CREATE TABLE IF NOT EXISTS beacon_api_eth_v2_beacon_block ON CLUSTER '{cluster}'
AS beacon_api_eth_v2_beacon_block_local
ENGINE = Distributed('{cluster}', currentDatabase(), 'beacon_api_eth_v2_beacon_block_local', cityHash64(slot_start_date_time, meta_network_name, meta_client_name, block_root, parent_root, state_root))
COMMENT 'Contains beacon API /eth/v2/beacon/blocks/{block_id} data from each sentry client attached to a beacon node.';

CREATE TABLE IF NOT EXISTS beacon_api_eth_v3_validator_block ON CLUSTER '{cluster}'
AS beacon_api_eth_v3_validator_block_local
ENGINE = Distributed('{cluster}', currentDatabase(), 'beacon_api_eth_v3_validator_block_local', cityHash64(slot_start_date_time, meta_network_name, meta_client_name, event_date_time))
COMMENT 'Contains beacon API /eth/v3/validator/blocks/{slot} data from each sentry client attached to a beacon node.';

CREATE TABLE IF NOT EXISTS beacon_api_slot ON CLUSTER '{cluster}'
AS beacon_api_slot_local
ENGINE = Distributed('{cluster}', currentDatabase(), 'beacon_api_slot_local', cityHash64(slot_start_date_time, slot))
COMMENT 'Aggregated beacon API slot data. Each row represents a slot from each sentry client attached to a beacon node.';

CREATE TABLE IF NOT EXISTS beacon_block_classification ON CLUSTER '{cluster}'
AS beacon_block_classification_local
ENGINE = Distributed('{cluster}', currentDatabase(), 'beacon_block_classification_local', cityHash64(slot_start_date_time, meta_network_name, meta_client_name, proposer_index))
COMMENT 'Contains beacon block classification for a given slot. This is a best guess based on the client probabilities of the proposer. This is not guaranteed to be correct.';

CREATE TABLE IF NOT EXISTS blob_submitter ON CLUSTER '{cluster}'
AS blob_submitter_local
ENGINE = Distributed('{cluster}', currentDatabase(), 'blob_submitter_local', cityHash64(address, meta_network_name))
COMMENT 'Contains blob submitter address to name mappings.';

CREATE TABLE IF NOT EXISTS block_native_mempool_transaction ON CLUSTER '{cluster}'
AS block_native_mempool_transaction_local
ENGINE = Distributed('{cluster}', currentDatabase(), 'block_native_mempool_transaction_local', cityHash64(detecttime, network, hash, fromaddress, nonce, gas))
COMMENT 'Contains transactions from block native mempool dataset';

CREATE TABLE IF NOT EXISTS canonical_beacon_blob_sidecar ON CLUSTER '{cluster}'
AS canonical_beacon_blob_sidecar_local
ENGINE = Distributed('{cluster}', currentDatabase(), 'canonical_beacon_blob_sidecar_local', cityHash64(slot_start_date_time, meta_network_name, block_root, blob_index))
COMMENT 'Contains a blob sidecar from a beacon block.';

CREATE TABLE IF NOT EXISTS canonical_beacon_block ON CLUSTER '{cluster}'
AS canonical_beacon_block_local
ENGINE = Distributed('{cluster}', currentDatabase(), 'canonical_beacon_block_local', cityHash64(slot_start_date_time, meta_network_name))
COMMENT 'Contains beacon block from a beacon node.';

CREATE TABLE IF NOT EXISTS canonical_beacon_block_attester_slashing ON CLUSTER '{cluster}'
AS canonical_beacon_block_attester_slashing_local
ENGINE = Distributed('{cluster}', currentDatabase(), 'canonical_beacon_block_attester_slashing_local', cityHash64(slot_start_date_time, meta_network_name, block_root, attestation_1_attesting_indices, attestation_2_attesting_indices, attestation_1_data_slot, attestation_2_data_slot, attestation_1_data_beacon_block_root, attestation_2_data_beacon_block_root))
COMMENT 'Contains attester slashing from a beacon block.';

CREATE TABLE IF NOT EXISTS canonical_beacon_block_bls_to_execution_change ON CLUSTER '{cluster}'
AS canonical_beacon_block_bls_to_execution_change_local
ENGINE = Distributed('{cluster}', currentDatabase(), 'canonical_beacon_block_bls_to_execution_change_local', cityHash64(slot_start_date_time, meta_network_name, block_root, exchanging_message_validator_index, exchanging_message_from_bls_pubkey, exchanging_message_to_execution_address))
COMMENT 'Contains bls to execution change from a beacon block.';

CREATE TABLE IF NOT EXISTS canonical_beacon_block_deposit ON CLUSTER '{cluster}'
AS canonical_beacon_block_deposit_local
ENGINE = Distributed('{cluster}', currentDatabase(), 'canonical_beacon_block_deposit_local', cityHash64(slot_start_date_time, meta_network_name, block_root, deposit_data_pubkey, deposit_proof))
COMMENT 'Contains a deposit from a beacon block.';

CREATE TABLE IF NOT EXISTS canonical_beacon_block_execution_transaction ON CLUSTER '{cluster}'
AS canonical_beacon_block_execution_transaction_local
ENGINE = Distributed('{cluster}', currentDatabase(), 'canonical_beacon_block_execution_transaction_local', cityHash64(slot_start_date_time, meta_network_name, block_root, position, hash, nonce))
COMMENT 'Contains execution transaction from a beacon block.';

CREATE TABLE IF NOT EXISTS canonical_beacon_block_proposer_slashing ON CLUSTER '{cluster}'
AS canonical_beacon_block_proposer_slashing_local
...
CREATE TABLE IF NOT EXISTS canonical_beacon_block_sync_aggregate ON CLUSTER '{cluster}'
AS canonical_beacon_block_sync_aggregate_local
ENGINE = Distributed('{cluster}', currentDatabase(), 'canonical_beacon_block_sync_aggregate_local', cityHash64(slot_start_date_time, meta_network_name, slot))
COMMENT 'Contains canonical beacon block sync aggregate data with expanded validator participation.';

CREATE TABLE IF NOT EXISTS canonical_beacon_block_voluntary_exit ON CLUSTER '{cluster}'
AS canonical_beacon_block_voluntary_exit_local
ENGINE = Distributed('{cluster}', currentDatabase(), 'canonical_beacon_block_voluntary_exit_local', cityHash64(slot_start_date_time, meta_network_name, block_root, voluntary_exit_message_epoch, voluntary_exit_message_validator_index))
COMMENT 'Contains a voluntary exit from a beacon block.';

CREATE TABLE IF NOT EXISTS canonical_beacon_block_withdrawal ON CLUSTER '{cluster}'
AS canonical_beacon_block_withdrawal_local
ENGINE = Distributed('{cluster}', currentDatabase(), 'canonical_beacon_block_withdrawal_local', cityHash64(slot_start_date_time, meta_network_name, block_root, withdrawal_index, withdrawal_validator_index))
COMMENT 'Contains a withdrawal from a beacon block.';

CREATE TABLE IF NOT EXISTS canonical_beacon_committee ON CLUSTER '{cluster}'
AS canonical_beacon_committee_local
ENGINE = Distributed('{cluster}', currentDatabase(), 'canonical_beacon_committee_local', cityHash64(slot_start_date_time, meta_network_name, committee_index))
COMMENT 'Contains canonical beacon API /eth/v1/beacon/committees data.';

CREATE TABLE IF NOT EXISTS canonical_beacon_elaborated_attestation ON CLUSTER '{cluster}'
AS canonical_beacon_elaborated_attestation_local
ENGINE = Distributed('{cluster}', currentDatabase(), 'canonical_beacon_elaborated_attestation_local', cityHash64(slot_start_date_time, meta_network_name, block_root, block_slot, position_in_block, beacon_block_root, slot, committee_index, source_root, target_root))
COMMENT 'Contains elaborated attestations from beacon blocks.';

CREATE TABLE IF NOT EXISTS canonical_beacon_proposer_duty ON CLUSTER '{cluster}'
AS canonical_beacon_proposer_duty_local
ENGINE = Distributed('{cluster}', currentDatabase(), 'canonical_beacon_proposer_duty_local', cityHash64(slot_start_date_time, meta_network_name, proposer_validator_index, proposer_pubkey))
COMMENT 'Contains a proposer duty from a beacon block.';

CREATE TABLE IF NOT EXISTS canonical_beacon_sync_committee ON CLUSTER '{cluster}'
AS canonical_beacon_sync_committee_local
ENGINE = Distributed('{cluster}', currentDatabase(), 'canonical_beacon_sync_committee_local', cityHash64(epoch_start_date_time, meta_network_name, sync_committee_period))
COMMENT 'Contains canonical beacon API /eth/v1/beacon/states/{state_id}/sync_committees data.';

CREATE TABLE IF NOT EXISTS canonical_beacon_validators ON CLUSTER '{cluster}'
AS canonical_beacon_validators_local
ENGINE = Distributed('{cluster}', currentDatabase(), 'canonical_beacon_validators_local', cityHash64(epoch_start_date_time, meta_network_name, index, status))
COMMENT 'Contains a validator state for an epoch.';

CREATE TABLE IF NOT EXISTS canonical_beacon_validators_pubkeys ON CLUSTER '{cluster}'
AS canonical_beacon_validators_pubkeys_local
ENGINE = Distributed('{cluster}', currentDatabase(), 'canonical_beacon_validators_pubkeys_local', cityHash64(index, meta_network_name))
COMMENT 'Contains a validator pubkeys for an epoch.';

CREATE TABLE IF NOT EXISTS canonical_beacon_validators_withdrawal_credentials ON CLUSTER '{cluster}'
AS canonical_beacon_validators_withdrawal_credentials_local
ENGINE = Distributed('{cluster}', currentDatabase(), 'canonical_beacon_validators_withdrawal_credentials_local', cityHash64(index, meta_network_name))
COMMENT 'Contains a validator withdrawal credentials for an epoch.';

CREATE TABLE IF NOT EXISTS canonical_execution_address_appearances ON CLUSTER '{cluster}'
AS canonical_execution_address_appearances_local
ENGINE = Distributed('{cluster}', currentDatabase(), 'canonical_execution_address_appearances_local', cityHash64(block_number, meta_network_name, transaction_hash))
COMMENT 'Contains canonical execution address appearance data.';

CREATE TABLE IF NOT EXISTS canonical_execution_balance_diffs ON CLUSTER '{cluster}'
AS canonical_execution_balance_diffs_local
ENGINE = Distributed('{cluster}', currentDatabase(), 'canonical_execution_balance_diffs_local', cityHash64(block_number, meta_network_name, transaction_hash))
COMMENT 'Contains canonical execution balance diff data.';

CREATE TABLE IF NOT EXISTS canonical_execution_balance_reads ON CLUSTER '{cluster}'
AS canonical_execution_balance_reads_local
ENGINE = Distributed('{cluster}', currentDatabase(), 'canonical_execution_balance_reads_local', cityHash64(block_number, meta_network_name, transaction_hash))
COMMENT 'Contains canonical execution balance read data.';

CREATE TABLE IF NOT EXISTS canonical_execution_block ON CLUSTER '{cluster}'
AS canonical_execution_block_local
ENGINE = Distributed('{cluster}', currentDatabase(), 'canonical_execution_block_local', cityHash64(block_number, meta_network_name))
COMMENT 'Contains canonical execution block data.';

CREATE TABLE IF NOT EXISTS canonical_execution_contracts ON CLUSTER '{cluster}'
AS canonical_execution_contracts_local
ENGINE = Distributed('{cluster}', currentDatabase(), 'canonical_execution_contracts_local', cityHash64(block_number, meta_network_name, transaction_hash))
COMMENT 'Contains canonical execution contract data.';

CREATE TABLE IF NOT EXISTS canonical_execution_erc20_transfers ON CLUSTER '{cluster}'
AS canonical_execution_erc20_transfers_local
ENGINE = Distributed('{cluster}', currentDatabase(), 'canonical_execution_erc20_transfers_local', cityHash64(block_number, meta_network_name, transaction_hash))
COMMENT 'Contains canonical execution erc20 transfer data.';

CREATE TABLE IF NOT EXISTS canonical_execution_erc721_transfers ON CLUSTER '{cluster}'
AS canonical_execution_erc721_transfers_local
ENGINE = Distributed('{cluster}', currentDatabase(), 'canonical_execution_erc721_transfers_local', cityHash64(block_number, meta_network_name, transaction_hash))
COMMENT 'Contains canonical execution erc721 transfer data.';

CREATE TABLE IF NOT EXISTS canonical_execution_four_byte_counts ON CLUSTER '{cluster}'
AS canonical_execution_four_byte_counts_local
ENGINE = Distributed('{cluster}', currentDatabase(), 'canonical_execution_four_byte_counts_local', cityHash64(block_number, meta_network_name, transaction_hash))
COMMENT 'Contains canonical execution four byte count data.';

CREATE TABLE IF NOT EXISTS canonical_execution_logs ON CLUSTER '{cluster}'
AS canonical_execution_logs_local
ENGINE = Distributed('{cluster}', currentDatabase(), 'canonical_execution_logs_local', cityHash64(block_number, meta_network_name, transaction_hash))
COMMENT 'Contains canonical execution logs data.';

CREATE TABLE IF NOT EXISTS canonical_execution_native_transfers ON CLUSTER '{cluster}'
AS canonical_execution_native_transfers_local
ENGINE = Distributed('{cluster}', currentDatabase(), 'canonical_execution_native_transfers_local', cityHash64(block_number, meta_network_name, transaction_hash))
COMMENT 'Contains canonical execution native transfer data.';

CREATE TABLE IF NOT EXISTS canonical_execution_nonce_diffs ON CLUSTER '{cluster}'
AS canonical_execution_nonce_diffs_local
ENGINE = Distributed('{cluster}', currentDatabase(), 'canonical_execution_nonce_diffs_local', cityHash64(block_number, meta_network_name, transaction_hash))
COMMENT 'Contains canonical execution nonce diff data.';

CREATE TABLE IF NOT EXISTS canonical_execution_nonce_reads ON CLUSTER '{cluster}'
AS canonical_execution_nonce_reads_local
ENGINE = Distributed('{cluster}', currentDatabase(), 'canonical_execution_nonce_reads_local', cityHash64(block_number, meta_network_name, transaction_hash))
COMMENT 'Contains canonical execution nonce read data.';

CREATE TABLE IF NOT EXISTS canonical_execution_storage_diffs ON CLUSTER '{cluster}'
AS canonical_execution_storage_diffs_local
ENGINE = Distributed('{cluster}', currentDatabase(), 'canonical_execution_storage_diffs_local', cityHash64(block_number, meta_network_name, transaction_hash))
COMMENT 'Contains canonical execution storage diffs data.';

CREATE TABLE IF NOT EXISTS canonical_execution_storage_reads ON CLUSTER '{cluster}'
AS canonical_execution_storage_reads_local
ENGINE = Distributed('{cluster}', currentDatabase(), 'canonical_execution_storage_reads_local', cityHash64(block_number, meta_network_name, transaction_hash))
COMMENT 'Contains canonical execution storage reads data.';

CREATE TABLE IF NOT EXISTS canonical_execution_traces ON CLUSTER '{cluster}'
AS canonical_execution_traces_local
ENGINE = Distributed('{cluster}', currentDatabase(), 'canonical_execution_traces_local', cityHash64(block_number, meta_network_name, transaction_hash))
COMMENT 'Contains canonical execution traces data.';

CREATE TABLE IF NOT EXISTS canonical_execution_transaction ON CLUSTER '{cluster}'
AS canonical_execution_transaction_local
ENGINE = Distributed('{cluster}', currentDatabase(), 'canonical_execution_transaction_local', cityHash64(block_number, meta_network_name, transaction_hash))
COMMENT 'Contains canonical execution transaction data.';

CREATE TABLE IF NOT EXISTS canonical_execution_transaction_structlog ON CLUSTER '{cluster}'
AS canonical_execution_transaction_structlog_local
ENGINE = Distributed('{cluster}', currentDatabase(), 'canonical_execution_transaction_structlog_local', cityHash64(block_number, meta_network_name, transaction_hash, index))
COMMENT 'Contains canonical execution transaction structlog data.';

CREATE TABLE IF NOT EXISTS canonical_execution_transaction_structlog_agg ON CLUSTER '{cluster}'
AS canonical_execution_transaction_structlog_agg_local
ENGINE = Distributed('{cluster}', currentDatabase(), 'canonical_execution_transaction_structlog_agg_local', cityHash64(block_number, meta_network_name, transaction_hash, call_frame_id))
COMMENT 'Aggregated EVM execution data. Summary rows (operation="") contain frame metadata. Per-opcode rows contain aggregated gas/count per (frame, opcode).';

CREATE TABLE IF NOT EXISTS consensus_engine_api_get_blobs ON CLUSTER '{cluster}'
AS consensus_engine_api_get_blobs_local
ENGINE = Distributed('{cluster}', currentDatabase(), 'consensus_engine_api_get_blobs_local', cityHash64(slot_start_date_time, meta_network_name, meta_client_name, block_root, event_date_time))
COMMENT 'Contains timing and instrumentation data for engine_getBlobs calls between the consensus and execution layer.';

CREATE TABLE IF NOT EXISTS consensus_engine_api_new_payload ON CLUSTER '{cluster}'
AS consensus_engine_api_new_payload_local
ENGINE = Distributed('{cluster}', currentDatabase(), 'consensus_engine_api_new_payload_local', cityHash64(slot_start_date_time, meta_network_name, meta_client_name, block_hash, event_date_time))
COMMENT 'Contains timing and instrumentation data for engine_newPayload calls between the consensus and execution layer.';

CREATE TABLE IF NOT EXISTS ethseer_validator_entity ON CLUSTER '{cluster}'
AS ethseer_validator_entity_local
ENGINE = Distributed('{cluster}', currentDatabase(), 'ethseer_validator_entity_local', cityHash64(index, pubkey, meta_network_name))
COMMENT 'Contains a mapping of validators to entities';

CREATE TABLE IF NOT EXISTS execution_block_metrics ON CLUSTER '{cluster}'
AS execution_block_metrics_local
ENGINE = Distributed('{cluster}', currentDatabase(), 'execution_block_metrics_local', cityHash64(block_number, meta_network_name, meta_client_name))
COMMENT 'Contains detailed performance metrics from execution client structured logging for block execution';

CREATE TABLE IF NOT EXISTS execution_engine_get_blobs ON CLUSTER '{cluster}'
AS execution_engine_get_blobs_local
ENGINE = Distributed('{cluster}', currentDatabase(), 'execution_engine_get_blobs_local', cityHash64(event_date_time, meta_network_name, meta_client_name))
COMMENT 'Contains timing and instrumentation data for engine_getBlobs calls from the execution layer perspective.';

CREATE TABLE IF NOT EXISTS execution_engine_new_payload ON CLUSTER '{cluster}'
AS execution_engine_new_payload_local
ENGINE = Distributed('{cluster}', currentDatabase(), 'execution_engine_new_payload_local', cityHash64(block_number, meta_network_name, meta_client_name, block_hash, event_date_time))
COMMENT 'Contains timing and instrumentation data for engine_newPayload calls from the execution layer perspective.';

CREATE TABLE IF NOT EXISTS execution_state_size ON CLUSTER '{cluster}'
AS execution_state_size_local
ENGINE = Distributed('{cluster}', currentDatabase(), 'execution_state_size_local', cityHash64(block_number, meta_network_name, meta_client_name, state_root, event_date_time))
COMMENT 'Contains execution layer state size metrics including account, contract code, and storage data measurements at specific block heights.';

CREATE TABLE IF NOT EXISTS execution_transaction ON CLUSTER '{cluster}'
AS execution_transaction_local
ENGINE = Distributed('{cluster}', currentDatabase(), 'execution_transaction_local', cityHash64(block_number, meta_network_name, block_hash, position))
COMMENT 'Contains execution transaction data that may not be canonical.';

CREATE TABLE IF NOT EXISTS imported_sources ON CLUSTER '{cluster}'
AS imported_sources_local
ENGINE = Distributed('{cluster}', currentDatabase(), 'imported_sources_local', rand())
COMMENT 'This table contains the list of sources that have been imported into the database';

CREATE TABLE IF NOT EXISTS libp2p_add_peer ON CLUSTER '{cluster}'
AS libp2p_add_peer_local
ENGINE = Distributed('{cluster}', currentDatabase(), 'libp2p_add_peer_local', cityHash64(event_date_time, meta_network_name, meta_client_name, peer_id_unique_key))
COMMENT 'Contains the details of the peers added to the libp2p client.';

CREATE TABLE IF NOT EXISTS libp2p_connected ON CLUSTER '{cluster}'
AS libp2p_connected_local
ENGINE = Distributed('{cluster}', currentDatabase(), 'libp2p_connected_local', cityHash64(event_date_time, meta_network_name, meta_client_name, remote_peer_id_unique_key, direction, opened))
COMMENT 'Contains the details of the CONNECTED events from the libp2p client.';

CREATE TABLE IF NOT EXISTS libp2p_deliver_message ON CLUSTER '{cluster}'
AS libp2p_deliver_message_local
ENGINE = Distributed('{cluster}', currentDatabase(), 'libp2p_deliver_message_local', cityHash64(event_date_time, meta_network_name, meta_client_name, peer_id_unique_key, topic_fork_digest_value, topic_name, message_id, seq_number))
COMMENT 'Contains the details of the DELIVER_MESSAGE events from the libp2p client.';

CREATE TABLE IF NOT EXISTS libp2p_disconnected ON CLUSTER '{cluster}'
AS libp2p_disconnected_local
ENGINE = Distributed('{cluster}', currentDatabase(), 'libp2p_disconnected_local', cityHash64(event_date_time, meta_network_name, meta_client_name, remote_peer_id_unique_key, direction, opened))
COMMENT 'Contains the details of the DISCONNECTED events from the libp2p client.';

CREATE TABLE IF NOT EXISTS libp2p_drop_rpc ON CLUSTER '{cluster}'
AS libp2p_drop_rpc_local
ENGINE = Distributed('{cluster}', currentDatabase(), 'libp2p_drop_rpc_local', unique_key)
COMMENT 'Contains the details of the RPC messages dropped by the peer.';

CREATE TABLE IF NOT EXISTS libp2p_duplicate_message ON CLUSTER '{cluster}'
AS libp2p_duplicate_message_local
ENGINE = Distributed('{cluster}', currentDatabase(), 'libp2p_duplicate_message_local', cityHash64(event_date_time, meta_network_name, meta_client_name, peer_id_unique_key, topic_fork_digest_value, topic_name, message_id, seq_number))
COMMENT 'Contains the details of the DUPLICATE_MESSAGE events from the libp2p client.';

CREATE TABLE IF NOT EXISTS libp2p_gossipsub_aggregate_and_proof ON CLUSTER '{cluster}'
AS libp2p_gossipsub_aggregate_and_proof_local
ENGINE = Distributed('{cluster}', currentDatabase(), 'libp2p_gossipsub_aggregate_and_proof_local', cityHash64(slot_start_date_time, meta_network_name, meta_client_name, peer_id_unique_key, message_id))
COMMENT 'Table for libp2p gossipsub aggregate and proof data.';

CREATE TABLE IF NOT EXISTS libp2p_gossipsub_beacon_attestation ON CLUSTER '{cluster}'
AS libp2p_gossipsub_beacon_attestation_local
ENGINE = Distributed('{cluster}', currentDatabase(), 'libp2p_gossipsub_beacon_attestation_local', cityHash64(slot_start_date_time, meta_network_name, meta_client_name, peer_id_unique_key, message_id))
COMMENT 'Table for libp2p gossipsub beacon attestation data.';

CREATE TABLE IF NOT EXISTS libp2p_gossipsub_beacon_block ON CLUSTER '{cluster}'
AS libp2p_gossipsub_beacon_block_local
ENGINE = Distributed('{cluster}', currentDatabase(), 'libp2p_gossipsub_beacon_block_local', cityHash64(slot_start_date_time, meta_network_name, meta_client_name, peer_id_unique_key, message_id))
COMMENT 'Table for libp2p gossipsub beacon block data.';

CREATE TABLE IF NOT EXISTS libp2p_gossipsub_blob_sidecar ON CLUSTER '{cluster}'
AS libp2p_gossipsub_blob_sidecar_local
ENGINE = Distributed('{cluster}', currentDatabase(), 'libp2p_gossipsub_blob_sidecar_local', cityHash64(slot_start_date_time, meta_network_name, meta_client_name, peer_id_unique_key, message_id))
COMMENT 'Table for libp2p gossipsub blob sidecar data';

CREATE TABLE IF NOT EXISTS libp2p_gossipsub_data_column_sidecar ON CLUSTER '{cluster}'
AS libp2p_gossipsub_data_column_sidecar_local
ENGINE = Distributed('{cluster}', currentDatabase(), 'libp2p_gossipsub_data_column_sidecar_local', cityHash64(slot_start_date_time, meta_network_name, meta_client_name, peer_id_unique_key, message_id))
COMMENT 'Table for libp2p gossipsub data column sidecar data';

CREATE TABLE IF NOT EXISTS libp2p_graft ON CLUSTER '{cluster}'
AS libp2p_graft_local
ENGINE = Distributed('{cluster}', currentDatabase(), 'libp2p_graft_local', cityHash64(event_date_time, meta_network_name, meta_client_name, peer_id_unique_key, topic_fork_digest_value, topic_name))
COMMENT 'Contains the details of the GRAFT events from the libp2p client.';

CREATE TABLE IF NOT EXISTS libp2p_handle_metadata ON CLUSTER '{cluster}'
AS libp2p_handle_metadata_local
ENGINE = Distributed('{cluster}', currentDatabase(), 'libp2p_handle_metadata_local', cityHash64(event_date_time, meta_network_name, meta_client_name, peer_id_unique_key, latency_milliseconds))
COMMENT 'Contains the metadata handling events for libp2p peers.';

CREATE TABLE IF NOT EXISTS libp2p_handle_status ON CLUSTER '{cluster}'
AS libp2p_handle_status_local
ENGINE = Distributed('{cluster}', currentDatabase(), 'libp2p_handle_status_local', cityHash64(event_date_time, meta_network_name, meta_client_name, peer_id_unique_key, latency_milliseconds))
COMMENT 'Contains the status handling events for libp2p peers.';

CREATE TABLE IF NOT EXISTS libp2p_identify ON CLUSTER '{cluster}'
AS libp2p_identify_local
ENGINE = Distributed('{cluster}', currentDatabase(), 'libp2p_identify_local', cityHash64(event_date_time, meta_network_name, meta_client_name, remote_peer_id_unique_key, direction))
COMMENT 'Contains libp2p identify protocol exchange results including remote peer agent info, supported protocols, and connection metadata';
...
CREATE TABLE IF NOT EXISTS libp2p_peer ON CLUSTER '{cluster}'
AS libp2p_peer_local
ENGINE = Distributed('{cluster}', currentDatabase(), 'libp2p_peer_local', unique_key)
COMMENT 'Contains the original peer id of a seahashed peer_id + meta_network_name, commonly seen in other tables as the field peer_id_unique_key';

CREATE TABLE IF NOT EXISTS libp2p_prune ON CLUSTER '{cluster}'
AS libp2p_prune_local
ENGINE = Distributed('{cluster}', currentDatabase(), 'libp2p_prune_local', cityHash64(event_date_time, meta_network_name, meta_client_name, peer_id_unique_key, topic_fork_digest_value, topic_name))
COMMENT 'Contains the details of the PRUNE events from the libp2p client.';

CREATE TABLE IF NOT EXISTS libp2p_publish_message ON CLUSTER '{cluster}'
AS libp2p_publish_message_local
ENGINE = Distributed('{cluster}', currentDatabase(), 'libp2p_publish_message_local', cityHash64(event_date_time, meta_network_name, meta_client_name, topic_fork_digest_value, topic_name, message_id))
COMMENT 'Contains the details of the PUBLISH_MESSAGE events from the libp2p client.';

CREATE TABLE IF NOT EXISTS libp2p_recv_rpc ON CLUSTER '{cluster}'
AS libp2p_recv_rpc_local
ENGINE = Distributed('{cluster}', currentDatabase(), 'libp2p_recv_rpc_local', unique_key)
COMMENT 'Contains the details of the RPC messages received by the peer.';

CREATE TABLE IF NOT EXISTS libp2p_reject_message ON CLUSTER '{cluster}'
AS libp2p_reject_message_local
ENGINE = Distributed('{cluster}', currentDatabase(), 'libp2p_reject_message_local', cityHash64(event_date_time, meta_network_name, meta_client_name, peer_id_unique_key, topic_fork_digest_value, topic_name, message_id, seq_number))
COMMENT 'Contains the details of the REJECT_MESSAGE events from the libp2p client.';

CREATE TABLE IF NOT EXISTS libp2p_remove_peer ON CLUSTER '{cluster}'
AS libp2p_remove_peer_local
ENGINE = Distributed('{cluster}', currentDatabase(), 'libp2p_remove_peer_local', cityHash64(event_date_time, meta_network_name, meta_client_name, peer_id_unique_key))
COMMENT 'Contains the details of the peers removed from the libp2p client.';
...
ENGINE = Distributed('{cluster}', currentDatabase(), 'libp2p_rpc_data_column_custody_probe_local', cityHash64(event_date_time, meta_network_name, meta_client_name, peer_id_unique_key, slot, column_index))
COMMENT 'Contains custody probe events for data column availability verification';

CREATE TABLE IF NOT EXISTS libp2p_rpc_meta_control_graft ON CLUSTER '{cluster}'
AS libp2p_rpc_meta_control_graft_local
ENGINE = Distributed('{cluster}', currentDatabase(), 'libp2p_rpc_meta_control_graft_local', unique_key)
COMMENT 'Contains the details of the "Graft" control messages from the peer.';

CREATE TABLE IF NOT EXISTS libp2p_rpc_meta_control_idontwant ON CLUSTER '{cluster}'
AS libp2p_rpc_meta_control_idontwant_local
ENGINE = Distributed('{cluster}', currentDatabase(), 'libp2p_rpc_meta_control_idontwant_local', unique_key)
COMMENT 'Contains the details of the IDONTWANT control messages from the peer.';

CREATE TABLE IF NOT EXISTS libp2p_rpc_meta_control_ihave ON CLUSTER '{cluster}'
AS libp2p_rpc_meta_control_ihave_local
ENGINE = Distributed('{cluster}', currentDatabase(), 'libp2p_rpc_meta_control_ihave_local', unique_key)
COMMENT 'Contains the details of the "I have" control messages from the peer.';

CREATE TABLE IF NOT EXISTS libp2p_rpc_meta_control_iwant ON CLUSTER '{cluster}'
AS libp2p_rpc_meta_control_iwant_local
ENGINE = Distributed('{cluster}', currentDatabase(), 'libp2p_rpc_meta_control_iwant_local', unique_key)
COMMENT 'Contains IWANT control messages from gossipsub. Collected from deep instrumentation within forked consensus layer clients. Peers request specific message IDs. Partition: monthly by `event_date_time`';

CREATE TABLE IF NOT EXISTS libp2p_rpc_meta_control_prune ON CLUSTER '{cluster}'
AS libp2p_rpc_meta_control_prune_local
ENGINE = Distributed('{cluster}', currentDatabase(), 'libp2p_rpc_meta_control_prune_local', unique_key)
COMMENT 'Contains the details of the "Prune" control messages from the peer.';

CREATE TABLE IF NOT EXISTS libp2p_rpc_meta_message ON CLUSTER '{cluster}'
AS libp2p_rpc_meta_message_local
ENGINE = Distributed('{cluster}', currentDatabase(), 'libp2p_rpc_meta_message_local', unique_key)
COMMENT 'Contains the details of the RPC meta messages from the peer';

CREATE TABLE IF NOT EXISTS libp2p_rpc_meta_subscription ON CLUSTER '{cluster}'
AS libp2p_rpc_meta_subscription_local
ENGINE = Distributed('{cluster}', currentDatabase(), 'libp2p_rpc_meta_subscription_local', unique_key)
COMMENT 'Contains the details of the RPC subscriptions from the peer.';

CREATE TABLE IF NOT EXISTS libp2p_send_rpc ON CLUSTER '{cluster}'
AS libp2p_send_rpc_local
ENGINE = Distributed('{cluster}', currentDatabase(), 'libp2p_send_rpc_local', unique_key)
COMMENT 'Contains the details of the RPC messages sent by the peer.';

CREATE TABLE IF NOT EXISTS libp2p_synthetic_heartbeat ON CLUSTER '{cluster}'
AS libp2p_synthetic_heartbeat_local
ENGINE = Distributed('{cluster}', currentDatabase(), 'libp2p_synthetic_heartbeat_local', cityHash64(event_date_time, meta_network_name, meta_client_name, remote_peer_id_unique_key, updated_date_time))
COMMENT 'Contains heartbeat events from libp2p peers';

CREATE TABLE IF NOT EXISTS mempool_dumpster_transaction ON CLUSTER '{cluster}'
AS mempool_dumpster_transaction_local
ENGINE = Distributed('{cluster}', currentDatabase(), 'mempool_dumpster_transaction_local', cityHash64(timestamp, chain_id, hash, from, nonce, gas))
COMMENT 'Contains transactions from mempool dumpster dataset. Following the parquet schema with some additions';

CREATE TABLE IF NOT EXISTS mempool_transaction ON CLUSTER '{cluster}'
AS mempool_transaction_local
ENGINE = Distributed('{cluster}', currentDatabase(), 'mempool_transaction_local', cityHash64(event_date_time, meta_network_name, meta_client_name, hash, from, nonce, gas))
COMMENT 'Each row represents a transaction that was seen in the mempool by a sentry client. Sentries can report the same transaction multiple times if it has been long enough since the last report.';

CREATE TABLE IF NOT EXISTS mev_relay_bid_trace ON CLUSTER '{cluster}'
AS mev_relay_bid_trace_local
ENGINE = Distributed('{cluster}', currentDatabase(), 'mev_relay_bid_trace_local', cityHash64(slot, meta_network_name))
COMMENT 'Contains MEV relay block bids data.';

CREATE TABLE IF NOT EXISTS mev_relay_proposer_payload_delivered ON CLUSTER '{cluster}'
AS mev_relay_proposer_payload_delivered_local
ENGINE = Distributed('{cluster}', currentDatabase(), 'mev_relay_proposer_payload_delivered_local', cityHash64(slot, meta_network_name))
COMMENT 'Contains MEV relay proposer payload delivered data.';

CREATE TABLE IF NOT EXISTS mev_relay_validator_registration ON CLUSTER '{cluster}'
AS mev_relay_validator_registration_local
ENGINE = Distributed('{cluster}', currentDatabase(), 'mev_relay_validator_registration_local', cityHash64(slot, meta_network_name))
COMMENT 'Contains MEV relay validator registrations data.';

CREATE TABLE IF NOT EXISTS node_record_consensus ON CLUSTER '{cluster}'
AS node_record_consensus_local
ENGINE = Distributed('{cluster}', currentDatabase(), 'node_record_consensus_local', cityHash64(event_date_time, meta_network_name, enr, meta_client_name))
COMMENT 'Contains consensus node records discovered by the Xatu discovery module.';

CREATE TABLE IF NOT EXISTS node_record_execution ON CLUSTER '{cluster}'
AS node_record_execution_local
ENGINE = Distributed('{cluster}', currentDatabase(), 'node_record_execution_local', cityHash64(event_date_time, meta_network_name, node_id, meta_client_name))
COMMENT 'Contains execution node records discovered by the Xatu discovery module.';

CREATE MATERIALIZED VIEW IF NOT EXISTS beacon_api_slot_attestation_mv_local ON CLUSTER '{cluster}' TO beacon_api_slot_local
(
    `slot` UInt32,
...
CREATE MATERIALIZED VIEW IF NOT EXISTS beacon_api_slot_block_mv_local ON CLUSTER '{cluster}' TO beacon_api_slot_local
(
    `slot` UInt32,
    `slot_start_date_time` DateTime,
    `epoch` UInt32,
