...
-- (the historical peer_id_unique_key values map straight into the new column).

-- 1. Stage existing libp2p_join rows into a temporary backup (old schema).
CREATE TABLE IF NOT EXISTS default.libp2p_join_migrate_bak_local ON CLUSTER '{cluster}'
(
...
CREATE TABLE IF NOT EXISTS default.libp2p_join_migrate_bak ON CLUSTER '{cluster}'
AS default.libp2p_join_migrate_bak_local
ENGINE = Distributed('{cluster}', 'default', 'libp2p_join_migrate_bak_local', cityHash64(event_date_time, meta_network_name, meta_client_name, peer_id_unique_key, topic_fork_digest_value, topic_name));

INSERT INTO default.libp2p_join_migrate_bak SELECT * FROM default.libp2p_join SETTINGS distributed_foreground_insert = 1;

-- 1b. Stage existing libp2p_leave rows into a temporary backup (old schema).
CREATE TABLE IF NOT EXISTS default.libp2p_leave_migrate_bak_local ON CLUSTER '{cluster}'
(
    `updated_date_time` DateTime COMMENT 'Timestamp when the record was last updated' CODEC(DoubleDelta, ZSTD(1)),
    `event_date_time` DateTime64(3) COMMENT 'Timestamp of the event' CODEC(DoubleDelta, ZSTD(1)),
    `topic_layer` LowCardinality(String) COMMENT 'Layer of the topic',
...
CREATE TABLE IF NOT EXISTS default.libp2p_leave_migrate_bak ON CLUSTER '{cluster}'
AS default.libp2p_leave_migrate_bak_local
ENGINE = Distributed('{cluster}', 'default', 'libp2p_leave_migrate_bak_local', cityHash64(event_date_time, meta_network_name, meta_client_name, peer_id_unique_key, topic_fork_digest_value, topic_name));

INSERT INTO default.libp2p_leave_migrate_bak SELECT * FROM default.libp2p_leave SETTINGS distributed_foreground_insert = 1;
...
-- 3. Recreate with the new schema (local_peer_id_unique_key), canonical ZK path.
...
FROM default.libp2p_join_migrate_bak SETTINGS distributed_foreground_insert = 1;

INSERT INTO default.libp2p_leave SELECT
    updated_date_time, event_date_time, topic_layer, topic_fork_digest_value, topic_name, topic_encoding,
    peer_id_unique_key AS local_peer_id_unique_key,
    meta_client_name, meta_client_version, meta_client_implementation, meta_client_os,
    meta_client_ip, meta_client_geo_city, meta_client_geo_country, meta_client_geo_country_code,
    meta_client_geo_continent_code, meta_client_geo_longitude, meta_client_geo_latitude,
    meta_client_geo_autonomous_system_number, meta_client_geo_autonomous_system_organization, meta_network_name
FROM default.libp2p_leave_migrate_bak SETTINGS distributed_foreground_insert = 1;

-- 5. Drop the temporary backups.
DROP TABLE IF EXISTS default.libp2p_join_migrate_bak ON CLUSTER '{cluster}' SYNC;
DROP TABLE IF EXISTS default.libp2p_join_migrate_bak_local ON CLUSTER '{cluster}' SYNC;
DROP TABLE IF EXISTS default.libp2p_leave_migrate_bak ON CLUSTER '{cluster}' SYNC;
DROP TABLE IF EXISTS default.libp2p_leave_migrate_bak_local ON CLUSTER '{cluster}' SYNC;
