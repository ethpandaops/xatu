-- Drop canonical_beacon_block_access_list tables
DROP TABLE IF EXISTS canonical_beacon_block_access_list ON CLUSTER '{cluster}';
DROP TABLE IF EXISTS canonical_beacon_block_access_list_local ON CLUSTER '{cluster}';

-- Remove columns from canonical_beacon_block
ALTER TABLE canonical_beacon_block ON CLUSTER '{cluster}'
    DROP COLUMN IF EXISTS execution_payload_block_access_list_root,
    DROP COLUMN IF EXISTS execution_payload_slot_number;

ALTER TABLE canonical_beacon_block_local ON CLUSTER '{cluster}'
    DROP COLUMN IF EXISTS execution_payload_block_access_list_root,
    DROP COLUMN IF EXISTS execution_payload_slot_number;

-- Remove columns from beacon_api_eth_v2_beacon_block
ALTER TABLE beacon_api_eth_v2_beacon_block ON CLUSTER '{cluster}'
    DROP COLUMN IF EXISTS execution_payload_slot_number;

ALTER TABLE beacon_api_eth_v2_beacon_block_local ON CLUSTER '{cluster}'
    DROP COLUMN IF EXISTS execution_payload_slot_number;
