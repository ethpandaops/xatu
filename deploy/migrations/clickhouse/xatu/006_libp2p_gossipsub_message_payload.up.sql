...
(
    ...
    `peer_id_unique_key` Int64 COMMENT 'Unique key for the peer that sent the message',
    ...
    `outcome` LowCardinality(String) COMMENT 'Gossip validation outcome for the message: deliver or reject',
    `reject_reason` LowCardinality(String) COMMENT 'Validation rejection reason, empty for delivered messages',
...
COMMENT 'Contains raw gossipsub message payloads keyed by message ID. Message IDs are content-derived, and the sorting key deliberately excludes client-specific columns so identical messages captured by multiple clients deduplicate on merge.';
