...
(
    ...
    `position_in_queue` UInt32 COMMENT 'The index of the deposit within the pending deposits queue' CODEC(ZSTD(1)),
    `pubkey` String COMMENT 'The public key of the validator from the pending deposit' CODEC(ZSTD(1)),
    `withdrawal_credentials` String COMMENT 'The withdrawal credentials from the pending deposit' CODEC(ZSTD(1)),
    `amount` UInt64 COMMENT 'The deposit amount in gwei' CODEC(ZSTD(1)),
    `signature` String COMMENT 'The deposit signature' CODEC(ZSTD(1)),
    `slot` UInt64 COMMENT 'The slot at which the deposit was queued' CODEC(DoubleDelta, ZSTD(1)),
