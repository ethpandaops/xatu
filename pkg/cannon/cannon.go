...
func (c *Cannon) startBeaconBlockProcessor(ctx context.Context) error {
	c.beacon.OnReady(ctx, func(ctx context.Context) error {
		...
		eventDerivers := []deriver.EventDeriver{
			...
			v2.NewConsolidationRequestDeriver(
				c.log,
				&c.Config.Derivers.ConsolidationRequestConfig,
				iterator.NewBackfillingCheckpoint(
					c.log,
					networkName,
					networkID,
					xatu.CannonType_BEACON_API_ETH_V2_BEACON_BLOCK_CONSOLIDATION_REQUEST,
					c.coordinatorClient,
					wallclock,
					&backfillingCheckpointIteratorMetrics,
					c.beacon,
					finalizedCheckpoint,
					3,
					&c.Config.Derivers.ConsolidationRequestConfig.Iterator,
				),
				c.beacon,
				clientMeta,
			),
