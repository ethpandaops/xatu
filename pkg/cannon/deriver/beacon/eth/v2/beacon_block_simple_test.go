...
func TestBeaconBlockDeriver_Name(t *testing.T) {
	...
	deriver := &BeaconBlockDeriver{
		log: logrus.NewEntry(logrus.New()),
		cfg: config,
		clientMeta: clientMeta,
	}

	assert.Equal(t, BeaconBlockDeriverName.String(), deriver.Name())
}

func TestBeaconBlockDeriver_CannonType(t *testing.T) {
	config := &BeaconBlockDeriverConfig{
		Enabled: true,
	}
	...
	assert.Equal(t, BeaconBlockDeriverName, deriver.CannonType())
}

func TestBeaconBlockDeriver_ActivationFork(t *testing.T) {
	config := &BeaconBlockDeriverConfig{
		Enabled: true,
	}
...
func TestBeaconBlockDeriver_OnEventsDerived(t *testing.T) {
	config := &BeaconBlockDeriverConfig{
		Enabled: true,
	}
...
func TestBeaconBlockDeriverConfig_Validation(t *testing.T) {
	tests := []struct {
		name   string
		config *BeaconBlockDeriverConfig
		valid  bool
...
func TestNewBeaconBlockDeriver(t *testing.T) {
	config := &BeaconBlockDeriverConfig{
		Enabled: true,
	}
	...
	deriver := NewBeaconBlockDeriver(log, config, nil, nil, clientMeta)
...
// Test the EventDeriver interface compliance
func TestBeaconBlockDeriver_ImplementsEventDeriver(t *testing.T) {
	config := &BeaconBlockDeriverConfig{
		Enabled: true,
	}
	...
	assert.IsType(t, xatu.CannonType_BEACON_API_ETH_V2_BEACON_BLOCK, deriver.CannonType())
...
func TestBeaconBlockDeriver_Constants(t *testing.T) {
	// Test that constants are properly defined
	assert.Equal(t, xatu.CannonType_BEACON_API_ETH_V2_BEACON_BLOCK, BeaconBlockDeriverName)
	assert.NotEmpty(t, BeaconBlockDeriverName.String())
}