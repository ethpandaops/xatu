...
func TestSnapshot_canonical_beacon_attestation_reward(t *testing.T) {
	if len(canonicalBeaconAttestationRewardEventNames) == 0 {
		t.Skip("no event names registered for canonical_beacon_attestation_reward")
	}

	testfixture.AssertSnapshot(t, newcanonicalBeaconAttestationRewardBatch(), &xatu.DecoratedEvent{
		Event: &xatu.Event{
			Name:     canonicalBeaconAttestationRewardEventNames[0],
