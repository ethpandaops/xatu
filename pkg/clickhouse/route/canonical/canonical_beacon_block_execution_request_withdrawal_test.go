...
func TestSnapshot_canonical_beacon_block_execution_request_withdrawal(t *testing.T) {
	if len(canonicalBeaconBlockExecutionRequestWithdrawalEventNames) == 0 {
		t.Skip("no event names registered for canonical_beacon_block_execution_request_withdrawal")
	}

	testfixture.AssertSnapshot(t, newcanonicalBeaconBlockExecutionRequestWithdrawalBatch(), &xatu.DecoratedEvent{
		Event: &xatu.Event{
			Name:     canonicalBeaconBlockExecutionRequestWithdrawalEventNames[0],
