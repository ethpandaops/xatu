...
func TestSnapshot_canonical_beacon_block_reward(t *testing.T) {
	if len(canonicalBeaconBlockRewardEventNames) == 0 {
		t.Skip("no event names registered for canonical_beacon_block_reward")
	}

	testfixture.AssertSnapshot(t, newcanonicalBeaconBlockRewardBatch(), &xatu.DecoratedEvent{
		Event: &xatu.Event{
			Name:     canonicalBeaconBlockRewardEventNames[0],
