...
func TestSnapshot_canonical_beacon_state_randao(t *testing.T) {
	if len(canonicalBeaconStateRandaoEventNames) == 0 {
		t.Skip("no event names registered for canonical_beacon_state_randao")
	}

	testfixture.AssertSnapshot(t, newcanonicalBeaconStateRandaoBatch(), &xatu.DecoratedEvent{
		Event: &xatu.Event{
			Name:     canonicalBeaconStateRandaoEventNames[0],
