...
func TestResolveProtoGen(t *testing.T) {
	tests := []struct {
		...
		// FixedString.
		{"FixedString(66)", "route.SafeColFixedStr", false, "nil", "ref"},
		{"FixedString(1)", "route.SafeColFixedStr", false, "nil", "ref"},
...
func TestResolveProtoGenSnapshotFmt(t *testing.T) {
	tests := []struct {
		...
		{"UUID", "%s.String()"},
		{"UInt128", "route.UInt128ToString(%s)"},
		{"UInt256", "route.UInt256ToString(%s)"},
		{"FixedString(66)", "string(%s)"},
		{"Decimal64(3)", "route.FormatDecimal(int64(%s), 3)"},
		{"Decimal(18, 9)", "route.FormatDecimal(int64(%s), 9)"},
		// Array(UInt8) needs base64 workaround.
		{"Array(UInt8)", "route.Uint8SliceToIntSlice(%s)"},
		// Array(FixedString) needs byte-to-string conversion.
		{"Array(FixedString(66))", "route.ByteSlicesToStrings(%s)"},
