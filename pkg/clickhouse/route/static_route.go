...
func NewStaticRoute(
	...
	if table == "" {
		return nil, fmt.Errorf("static route has empty table")
	}

	if len(events) == 0 {
		return nil, fmt.Errorf("static route has no events")
	}

	if batchFactory == nil {
		return nil, fmt.Errorf("static route has nil batch factory")
	}
