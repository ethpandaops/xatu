...
func sharedTestMetrics() *telemetry.Metrics {
	testMetricsOnce.Do(func() {
		testMetrics = telemetry.NewMetrics("test_chgo", "writer")
	})

	return testMetrics
}

// newTestWriter builds a minimal Writer suitable for unit tests that
// exercise doWithRetry and getOrCreateTableWriter without a real ClickHouse.
func newTestWriter(maxRetries int, baseDelay, maxDelay time.Duration) *Writer {
	log := logrus.New()
