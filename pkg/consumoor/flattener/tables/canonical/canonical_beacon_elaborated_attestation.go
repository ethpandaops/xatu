...
func (CanonicalBeaconElaboratedAttestationRoute) Table() flattener.TableName {
	return flattener.TableName("canonical_beacon_elaborated_attestation")
}

func (r CanonicalBeaconElaboratedAttestationRoute) Build() flattener.Route {
	return flattener.
		From(xatu.Event_BEACON_API_ETH_V2_BEACON_BLOCK_ELABORATED_ATTESTATION).
		To(r.Table()).
		...
		Apply(flattener.ApplyExplicitAliases(map[string]string{"validator_indexes": "validators"})).
		Apply(flattener.NormalizeDateTimeValues).
		Build()
}

func init() {
	catalog.MustRegister(CanonicalBeaconElaboratedAttestationRoute{})
}
