...
const beaconApiEthV1EventsAttestationTableName route.TableName = "beacon_api_eth_v1_events_attestation"

type beaconApiEthV1EventsAttestationBatch struct {
	...
	AttestingValidatorIndex                   *proto.ColNullable[uint32]
	AttestingValidatorCommitteeIndex          proto.ColStr
...
func newbeaconApiEthV1EventsAttestationBatch() *beaconApiEthV1EventsAttestationBatch {
	return &beaconApiEthV1EventsAttestationBatch{
		EventDateTime:                             func() proto.ColDateTime64 { var c proto.ColDateTime64; c.WithPrecision(proto.Precision(3)); return c }(),
		AttestingValidatorIndex:                   new(proto.ColUInt32).Nullable(),
...
func (b *beaconApiEthV1EventsAttestationBatch) Rows() int {
	return b.rows
}

func (b *beaconApiEthV1EventsAttestationBatch) appendMetadata(event *xatu.DecoratedEvent) {
...
func (b *beaconApiEthV1EventsAttestationBatch) Input() proto.Input {
	return proto.Input{
		...
		{Name: "attesting_validator_index", Data: b.AttestingValidatorIndex},
		{Name: "attesting_validator_committee_index", Data: &b.AttestingValidatorCommitteeIndex},
...
func (b *beaconApiEthV1EventsAttestationBatch) Reset() {
	b.EventDateTime.Reset()
	b.Slot.Reset()
	b.SlotStartDateTime.Reset()
	b.PropagationSlotStartDiff.Reset()
	b.CommitteeIndex.Reset()
	b.AttestingValidatorIndex.Reset()
	b.AttestingValidatorCommitteeIndex.Reset()
...
func (b *beaconApiEthV1EventsAttestationBatch) Snapshot() []map[string]any {
	n := b.rows
	...
	for i := 0; i < n; i++ {
		...
		if v := b.AttestingValidatorIndex.Row(i); v.Set {
			row["attesting_validator_index"] = v.Value
		} else {
			row["attesting_validator_index"] = nil
		}
		row["attesting_validator_committee_index"] = b.AttestingValidatorCommitteeIndex.Row(i)
