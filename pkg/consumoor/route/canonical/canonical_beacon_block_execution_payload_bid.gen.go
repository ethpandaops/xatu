...
const canonicalBeaconBlockExecutionPayloadBidTableName route.TableName = "canonical_beacon_block_execution_payload_bid"
...
type canonicalBeaconBlockExecutionPayloadBidBatch struct {
	...
	ExecutionPayment                          proto.ColUInt64
	FeeRecipient                              route.SafeColFixedStr
	GasLimit                                  proto.ColUInt64
	PrevRandao                                route.SafeColFixedStr
	BlobKzgCommitmentCount                    proto.ColUInt32
...
func newcanonicalBeaconBlockExecutionPayloadBidBatch() *canonicalBeaconBlockExecutionPayloadBidBatch {
	return &canonicalBeaconBlockExecutionPayloadBidBatch{
		...
		PrevRandao:                          func() route.SafeColFixedStr { var c route.SafeColFixedStr; c.SetSize(66); return c }(),
...
func (b *canonicalBeaconBlockExecutionPayloadBidBatch) Rows() int {
	return b.rows
}

func (b *canonicalBeaconBlockExecutionPayloadBidBatch) appendMetadata(event *xatu.DecoratedEvent) {
...
func (b *canonicalBeaconBlockExecutionPayloadBidBatch) Input() proto.Input {
	return proto.Input{
		...
		{Name: "prev_randao", Data: &b.PrevRandao},
		{Name: "blob_kzg_commitment_count", Data: &b.BlobKzgCommitmentCount},
...
func (b *canonicalBeaconBlockExecutionPayloadBidBatch) Reset() {
	...
	b.GasLimit.Reset()
	b.PrevRandao.Reset()
	b.BlobKzgCommitmentCount.Reset()
...
func (b *canonicalBeaconBlockExecutionPayloadBidBatch) Snapshot() []map[string]any {
	n := b.rows
	...
	for i := 0; i < n; i++ {
		...
		row["execution_payment"] = b.ExecutionPayment.Row(i)
		row["fee_recipient"] = string(b.FeeRecipient.Row(i))
		row["gas_limit"] = b.GasLimit.Row(i)
		row["prev_randao"] = string(b.PrevRandao.Row(i))
		row["blob_kzg_commitment_count"] = b.BlobKzgCommitmentCount.Row(i)
