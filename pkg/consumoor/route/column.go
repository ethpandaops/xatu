...
// NormalizeIPToIPv6 parses an IP string and returns proto.IPv6.
// IPv4 inputs are mapped to IPv6 (::ffff:x.x.x.x) in a single parse
// without an intermediate string representation.
...
// ConsensusVersionMajor extracts the major component from a raw
// consensus version string.
func ConsensusVersionMajor(raw string) string {
	major, _, _ := parseConsensusVersion(raw)

	return major
}

// ConsensusVersionMinor extracts the minor component from a raw
// consensus version string.
func ConsensusVersionMinor(raw string) string {
	_, minor, _ := parseConsensusVersion(raw)

	return minor
}

// ConsensusVersionPatch extracts the patch component from a raw
// consensus version string.
func ConsensusVersionPatch(raw string) string {
	_, _, patch := parseConsensusVersion(raw)
