...
var routeIndex = func() map[string]route.Route {
	routes, err := tabledefs.All()
	if err != nil {
		panic("route registration errors: " + err.Error())
	}

	m := make(map[string]route.Route, len(routes))
