...
var libp2pGossipsubBlobSidecarEventNames = []xatu.Event_Name{
...
func init() {
	r, err := route.NewStaticRoute(
		...
		libp2pGossipsubBlobSidecarEventNames,
		func() route.ColumnarBatch { return newlibp2pGossipsubBlobSidecarBatch() },
	)
	if err != nil {
...
func (b *libp2pGossipsubBlobSidecarBatch) validate(event *xatu.DecoratedEvent) error {
	...
	if payload.GetIndex() == nil {
		return fmt.Errorf("nil BlobIndex: %w", route.ErrInvalidEvent)
	}
	...
	additional := event.GetMeta().GetClient().GetLibp2PTraceGossipsubBlobSidecar()
	if additional == nil {
...
func (b *libp2pGossipsubBlobSidecarBatch) appendRuntime(event *xatu.DecoratedEvent) {
...
func (b *libp2pGossipsubBlobSidecarBatch) appendClientAdditionalData(
	event *xatu.DecoratedEvent,
) {
