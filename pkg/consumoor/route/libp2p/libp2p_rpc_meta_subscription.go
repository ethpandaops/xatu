...
var libp2pRpcMetaSubscriptionEventNames = []xatu.Event_Name{
...
func init() {
	r, err := route.NewStaticRoute(
		...
		libp2pRpcMetaSubscriptionEventNames,
		func() route.ColumnarBatch { return newlibp2pRpcMetaSubscriptionBatch() },
	)
	if err != nil {
...
func (b *libp2pRpcMetaSubscriptionBatch) FlattenTo(
	...
	if event.GetLibp2PTraceRpcMetaSubscription() == nil {
		return fmt.Errorf("nil libp2p_trace_rpc_meta_subscription payload: %w", route.ErrInvalidEvent)
	}
...
func (b *libp2pRpcMetaSubscriptionBatch) appendRuntime(event *xatu.DecoratedEvent) {
...
func (b *libp2pRpcMetaSubscriptionBatch) appendPayload(
	event *xatu.DecoratedEvent,
) {
	...
	if rootEventID != "" {
		b.UniqueKey.Append(computeRPCMetaChildUniqueKey(
			rootEventID, "rpc_meta_subscription",
	...
	if subscribe := payload.GetSubscribe(); subscribe != nil {
		b.Subscribe.Append(subscribe.GetValue())
	} else {
		b.Subscribe.Append(false)
	}
