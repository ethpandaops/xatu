...
func init() {
	flattener.MustRegister(flattener.NewStaticRoute(
		...
		[]xatu.Event_Name{xatu.Event_BEACON_API_ETH_V1_EVENTS_BLOB_SIDECAR},
		func() flattener.ColumnarBatch {
			return newbeaconApiEthV1EventsBlobSidecarBatch()
		},
	))
}
...
func (b *beaconApiEthV1EventsBlobSidecarBatch) FlattenTo(
	...
	payload := event.GetEthV1EventsBlobSidecar()
	if payload == nil {
		return fmt.Errorf("nil EthV1EventsBlobSidecar payload: %w", flattener.ErrInvalidEvent)
	}
	...
	addl := event.GetMeta().GetClient().GetEthV1EventsBlobSidecar()
...
func (b *beaconApiEthV1EventsBlobSidecarBatch) validate(event *xatu.DecoratedEvent) error {
	payload := event.GetEthV1EventsBlobSidecar()
	...
	addl := event.GetMeta().GetClient().GetEthV1EventsBlobSidecar()
	if addl == nil {
		return fmt.Errorf("nil EthV1EventsBlobSidecar additional data: %w", flattener.ErrInvalidEvent)
	}
