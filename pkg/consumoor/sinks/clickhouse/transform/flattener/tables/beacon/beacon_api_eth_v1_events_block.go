...
func init() {
	flattener.MustRegister(flattener.NewStaticRoute(
		...
		[]xatu.Event_Name{xatu.Event_BEACON_API_ETH_V1_EVENTS_BLOCK_V2},
		func() flattener.ColumnarBatch {
			return newbeaconApiEthV1EventsBlockBatch()
		},
	))
}
...
func (b *beaconApiEthV1EventsBlockBatch) FlattenTo(
	...
	payload := event.GetEthV1EventsBlockV2()
	if payload == nil {
		return fmt.Errorf("nil EthV1EventsBlockV2 payload: %w", flattener.ErrInvalidEvent)
	}
	...
	addl := event.GetMeta().GetClient().GetEthV1EventsBlockV2()
...
func (b *beaconApiEthV1EventsBlockBatch) validate(event *xatu.DecoratedEvent) error {
	payload := event.GetEthV1EventsBlockV2()
	...
	addl := event.GetMeta().GetClient().GetEthV1EventsBlockV2()
	if addl == nil {
		return fmt.Errorf("nil EthV1EventsBlockV2 additional data: %w", flattener.ErrInvalidEvent)
	}
