...
func init() {
	flattener.MustRegister(flattener.NewStaticRoute(
		...
		[]xatu.Event_Name{xatu.Event_BEACON_API_ETH_V1_EVENTS_HEAD_V2},
		func() flattener.ColumnarBatch {
			return newbeaconApiEthV1EventsHeadBatch()
		},
	))
}
...
func (b *beaconApiEthV1EventsHeadBatch) FlattenTo(
	...
	payload := event.GetEthV1EventsHeadV2()
	if payload == nil {
		return fmt.Errorf("nil EthV1EventsHeadV2 payload: %w", flattener.ErrInvalidEvent)
	}
	...
	addl := event.GetMeta().GetClient().GetEthV1EventsHeadV2()
	...
	b.EpochTransition.Append(payload.GetEpochTransition())
	b.ExecutionOptimistic.Append(false)
	b.PreviousDutyDependentRoot.Append([]byte(payload.GetPreviousDutyDependentRoot()))
	b.CurrentDutyDependentRoot.Append([]byte(payload.GetCurrentDutyDependentRoot()))
...
func (b *beaconApiEthV1EventsHeadBatch) validate(event *xatu.DecoratedEvent) error {
	payload := event.GetEthV1EventsHeadV2()
	...
	addl := event.GetMeta().GetClient().GetEthV1EventsHeadV2()
	if addl == nil {
		return fmt.Errorf("nil EthV1EventsHeadV2 additional data: %w", flattener.ErrInvalidEvent)
	}
