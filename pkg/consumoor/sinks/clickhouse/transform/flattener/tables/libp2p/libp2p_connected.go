...
func init() {
	flattener.MustRegister(flattener.NewStaticRoute(
		...
		func() flattener.ColumnarBatch {
			return newlibp2pConnectedBatch()
		},
	))
}
...
func (b *libp2pConnectedBatch) FlattenTo(
	...
	addl := event.GetMeta().GetClient().GetLibp2PTraceConnected()
	...
	serverMeta := event.GetMeta().GetServer().GetLIBP2P_TRACE_CONNECTED()
...
func (b *libp2pConnectedBatch) validate(event *xatu.DecoratedEvent) error {
	addl := event.GetMeta().GetClient().GetLibp2PTraceConnected()
	if addl == nil {
		return fmt.Errorf("nil LibP2PTraceConnected additional data: %w", flattener.ErrInvalidEvent)
	}
...
func (b *libp2pConnectedBatch) appendRemoteGeo(serverMeta *xatu.ServerMeta_AdditionalLibp2PTraceConnectedData) {
