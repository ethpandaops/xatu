...
func init() {
	flattener.MustRegister(flattener.NewStaticRoute(
		...
		[]xatu.Event_Name{xatu.Event_LIBP2P_TRACE_DUPLICATE_MESSAGE},
		func() flattener.ColumnarBatch {
			return newlibp2pDuplicateMessageBatch()
		},
	))
}
...
func (b *libp2pDuplicateMessageBatch) FlattenTo(
	...
	if payload == nil {
		return fmt.Errorf("nil LibP2PTraceDuplicateMessage payload: %w", flattener.ErrInvalidEvent)
	}
	...
	addl := event.GetMeta().GetClient().GetLibp2PTraceDuplicateMessage()
...
func (b *libp2pDuplicateMessageBatch) validate(event *xatu.DecoratedEvent) error {
	addl := event.GetMeta().GetClient().GetLibp2PTraceDuplicateMessage()
	if addl == nil {
		return fmt.Errorf("nil LibP2PTraceDuplicateMessage additional data: %w", flattener.ErrInvalidEvent)
	}
