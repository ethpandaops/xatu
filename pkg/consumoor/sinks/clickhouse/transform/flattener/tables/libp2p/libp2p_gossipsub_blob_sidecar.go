...
func init() {
	flattener.MustRegister(flattener.NewStaticRoute(
		libp2pGossipsubBlobSidecarTableName,
		[]xatu.Event_Name{xatu.Event_LIBP2P_TRACE_GOSSIPSUB_BLOB_SIDECAR},
		func() flattener.ColumnarBatch {
			return newlibp2pGossipsubBlobSidecarBatch()
		},
	))
}

func (b *libp2pGossipsubBlobSidecarBatch) FlattenTo(
	...
	if payload == nil {
		return fmt.Errorf("nil LibP2PTraceGossipsubBlobSidecar payload: %w", flattener.ErrInvalidEvent)
	}
	...
	addl := event.GetMeta().GetClient().GetLibp2PTraceGossipsubBlobSidecar()
	...
	b.BlobIndex.Append(uint32(payload.GetIndex().GetValue()))                                      //nolint:gosec // G115: blob index fits uint32.
...
func (b *libp2pGossipsubBlobSidecarBatch) validate(event *xatu.DecoratedEvent) error {
	addl := event.GetMeta().GetClient().GetLibp2PTraceGossipsubBlobSidecar()
	if addl == nil {
		return fmt.Errorf("nil LibP2PTraceGossipsubBlobSidecar additional data: %w", flattener.ErrInvalidEvent)
	}
	...
	if payload.GetIndex() == nil {
		return fmt.Errorf("nil BlobIndex: %w", flattener.ErrInvalidEvent)
	}
