...
func init() {
	flattener.MustRegister(flattener.NewStaticRoute(
		libp2pJoinTableName,
		[]xatu.Event_Name{xatu.Event_LIBP2P_TRACE_JOIN},
		func() flattener.ColumnarBatch {
			return newlibp2pJoinBatch()
		},
	))
}
...
func (b *libp2pJoinBatch) FlattenTo(
	...
	if payload == nil {
		return fmt.Errorf("nil LibP2PTraceJoin payload: %w", flattener.ErrInvalidEvent)
	}
	...
	addl := event.GetMeta().GetClient().GetLibp2PTraceJoin()
...
func (b *libp2pJoinBatch) validate(event *xatu.DecoratedEvent) error {
	addl := event.GetMeta().GetClient().GetLibp2PTraceJoin()
	if addl == nil {
		return fmt.Errorf("nil LibP2PTraceJoin additional data: %w", flattener.ErrInvalidEvent)
	}
