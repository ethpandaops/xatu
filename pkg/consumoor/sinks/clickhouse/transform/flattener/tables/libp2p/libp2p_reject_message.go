...
func init() {
	flattener.MustRegister(flattener.NewStaticRoute(
		...
		[]xatu.Event_Name{xatu.Event_LIBP2P_TRACE_REJECT_MESSAGE},
		func() flattener.ColumnarBatch {
			return newlibp2pRejectMessageBatch()
		},
	))
}
...
func (b *libp2pRejectMessageBatch) FlattenTo(
	...
	if payload == nil {
		return fmt.Errorf("nil LibP2PTraceRejectMessage payload: %w", flattener.ErrInvalidEvent)
	}
	...
	addl := event.GetMeta().GetClient().GetLibp2PTraceRejectMessage()
	...
	b.Reason.Append(payload.GetReason().GetValue())
...
func (b *libp2pRejectMessageBatch) validate(event *xatu.DecoratedEvent) error {
	addl := event.GetMeta().GetClient().GetLibp2PTraceRejectMessage()
	if addl == nil {
		return fmt.Errorf("nil LibP2PTraceRejectMessage additional data: %w", flattener.ErrInvalidEvent)
	}
