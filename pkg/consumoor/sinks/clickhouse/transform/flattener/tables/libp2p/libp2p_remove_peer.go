...
func init() {
	flattener.MustRegister(flattener.NewStaticRoute(
		...
		[]xatu.Event_Name{xatu.Event_LIBP2P_TRACE_REMOVE_PEER},
		func() flattener.ColumnarBatch {
			return newlibp2pRemovePeerBatch()
		},
	))
}
...
func (b *libp2pRemovePeerBatch) FlattenTo(
	...
	if payload == nil {
		return fmt.Errorf("nil LibP2PTraceRemovePeer payload: %w", flattener.ErrInvalidEvent)
	}
	...
	addl := event.GetMeta().GetClient().GetLibp2PTraceRemovePeer()
...
func (b *libp2pRemovePeerBatch) validate(event *xatu.DecoratedEvent) error {
	addl := event.GetMeta().GetClient().GetLibp2PTraceRemovePeer()
	if addl == nil {
		return fmt.Errorf("nil LibP2PTraceRemovePeer additional data: %w", flattener.ErrInvalidEvent)
	}
