...
func init() {
	flattener.MustRegister(flattener.NewStaticRoute(
		...
		func() flattener.ColumnarBatch {
			return newlibp2pRpcMetaControlIdontwantBatch()
		},
	))
}
...
func (b *libp2pRpcMetaControlIdontwantBatch) FlattenTo(
	...
	if payload == nil {
		return fmt.Errorf("nil LibP2PTraceRpcMetaControlIdontwant payload: %w", flattener.ErrInvalidEvent)
	}
	...
	addl := event.GetMeta().GetClient().GetLibp2PTraceRpcMetaControlIdontwant()
...
func (b *libp2pRpcMetaControlIdontwantBatch) validate(event *xatu.DecoratedEvent) error {
	addl := event.GetMeta().GetClient().GetLibp2PTraceRpcMetaControlIdontwant()
	if addl == nil {
		return fmt.Errorf("nil LibP2PTraceRpcMetaControlIdontwant additional data: %w", flattener.ErrInvalidEvent)
	}
