...
func init() {
	flattener.MustRegister(flattener.NewStaticRoute(
		...
		func() flattener.ColumnarBatch {
			return newlibp2pRpcMetaControlIhaveBatch()
		},
	))
}
...
func (b *libp2pRpcMetaControlIhaveBatch) FlattenTo(
	...
	if payload == nil {
		return fmt.Errorf("nil LibP2PTraceRpcMetaControlIhave payload: %w", flattener.ErrInvalidEvent)
	}
	...
	addl := event.GetMeta().GetClient().GetLibp2PTraceRpcMetaControlIhave()
...
func (b *libp2pRpcMetaControlIhaveBatch) validate(event *xatu.DecoratedEvent) error {
	addl := event.GetMeta().GetClient().GetLibp2PTraceRpcMetaControlIhave()
	if addl == nil {
		return fmt.Errorf("nil LibP2PTraceRpcMetaControlIhave additional data: %w", flattener.ErrInvalidEvent)
	}
