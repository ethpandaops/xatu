...
const nodeRecordConsensusTableName flattener.TableName = "node_record_consensus"

type nodeRecordConsensusBatch struct {
	...
	ForkDigest                                proto.ColStr
	NextForkDigest                            *proto.ColNullable[string]
	FinalizedRoot                             proto.ColStr
	FinalizedEpoch                            proto.ColUInt64
	HeadRoot                                  proto.ColStr
	HeadSlot                                  proto.ColUInt64
	Cgc                                       *proto.ColNullable[string]
	FinalizedEpochStartDateTime               *proto.ColNullable[time.Time]
	HeadSlotStartDateTime                     *proto.ColNullable[time.Time]
	IP                                        *proto.ColNullable[proto.IPv6]
	Tcp                                       *proto.ColNullable[uint16]
	Udp                                       *proto.ColNullable[uint16]
	Quic                                      *proto.ColNullable[uint16]
...
func newnodeRecordConsensusBatch() *nodeRecordConsensusBatch {
	return &nodeRecordConsensusBatch{
		...
		FinalizedEpochStartDateTime:         new(proto.ColDateTime).Nullable(),
		HeadSlotStartDateTime:               new(proto.ColDateTime).Nullable(),
		IP:                                  new(proto.ColIPv6).Nullable(),
		Tcp:                                 new(proto.ColUInt16).Nullable(),
		Udp:                                 new(proto.ColUInt16).Nullable(),
		Quic:                                new(proto.ColUInt16).Nullable(),
...
func (b *nodeRecordConsensusBatch) Rows() int {
	return b.rows
}

func (b *nodeRecordConsensusBatch) appendMetadata(meta *metadata.CommonMetadata) {
	if meta == nil {
...
func (b *nodeRecordConsensusBatch) Input() proto.Input {
	return proto.Input{
		...
		{Name: "fork_digest", Data: &b.ForkDigest},
		{Name: "next_fork_digest", Data: b.NextForkDigest},
		{Name: "finalized_root", Data: &b.FinalizedRoot},
		{Name: "finalized_epoch", Data: &b.FinalizedEpoch},
		{Name: "head_root", Data: &b.HeadRoot},
		{Name: "head_slot", Data: &b.HeadSlot},
		{Name: "cgc", Data: b.Cgc},
		{Name: "finalized_epoch_start_date_time", Data: b.FinalizedEpochStartDateTime},
		{Name: "head_slot_start_date_time", Data: b.HeadSlotStartDateTime},
		{Name: "ip", Data: b.IP},
		{Name: "tcp", Data: b.Tcp},
		{Name: "udp", Data: b.Udp},
		{Name: "quic", Data: b.Quic},
...
func (b *nodeRecordConsensusBatch) Reset() {
	...
	b.ForkDigest.Reset()
	b.NextForkDigest.Reset()
	b.FinalizedRoot.Reset()
	b.FinalizedEpoch.Reset()
	b.HeadRoot.Reset()
	b.HeadSlot.Reset()
	b.Cgc.Reset()
	b.FinalizedEpochStartDateTime.Reset()
	b.HeadSlotStartDateTime.Reset()
	b.IP.Reset()
	b.Tcp.Reset()
	b.Udp.Reset()
	b.Quic.Reset()
...
func (b *nodeRecordConsensusBatch) Snapshot() []map[string]any {
	n := b.rows
	...
	for i := 0; i < n; i++ {
		...
		row["fork_digest"] = b.ForkDigest.Row(i)
		if v := b.NextForkDigest.Row(i); v.Set {
			row["next_fork_digest"] = v.Value
		} else {
			row["next_fork_digest"] = nil
		}
		row["finalized_root"] = b.FinalizedRoot.Row(i)
		row["finalized_epoch"] = b.FinalizedEpoch.Row(i)
		row["head_root"] = b.HeadRoot.Row(i)
		row["head_slot"] = b.HeadSlot.Row(i)
		if v := b.Cgc.Row(i); v.Set {
			row["cgc"] = v.Value
		} else {
			row["cgc"] = nil
		}
		if v := b.FinalizedEpochStartDateTime.Row(i); v.Set {
			row["finalized_epoch_start_date_time"] = v.Value.Unix()
		} else {
			row["finalized_epoch_start_date_time"] = nil
		}
		if v := b.HeadSlotStartDateTime.Row(i); v.Set {
			row["head_slot_start_date_time"] = v.Value.Unix()
		} else {
			row["head_slot_start_date_time"] = nil
		}
