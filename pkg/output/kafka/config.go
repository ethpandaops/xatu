...
// sink-specific fields like Topic/TopicPattern and batching parameters.
//
// Exactly one of Topic or TopicPattern must be set. TopicPattern supports
// template variables ${EVENT_NAME} (SCREAMING_SNAKE) and ${event-name} (kebab).
type Config struct {
...
func (c *Config) Validate() error {
	...
	if c.Topic == "" && c.TopicPattern == "" {
		return errors.New("exactly one of topic or topicPattern is required")
	}

	if c.Topic != "" && c.TopicPattern != "" {
		return errors.New("topic and topicPattern are mutually exclusive")
	}
