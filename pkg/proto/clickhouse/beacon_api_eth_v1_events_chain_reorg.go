...
// SQL query builder for beacon_api_eth_v1_events_chain_reorg
...
// BuildListBeaconApiEthV1EventsChainReorgQuery constructs a parameterized SQL query from a ListBeaconApiEthV1EventsChainReorgRequest
func BuildListBeaconApiEthV1EventsChainReorgQuery(req *ListBeaconApiEthV1EventsChainReorgRequest, options ...QueryOption) (SQLQuery, error) {
	...
	// Add filter for column: old_head_block
	if req.OldHeadBlock != nil {
		switch filter := req.OldHeadBlock.Filter.(type) {
		case *StringFilter_Eq:
			qb.AddCondition("old_head_block", "=", filter.Eq)
		case *StringFilter_Ne:
			qb.AddCondition("old_head_block", "!=", filter.Ne)
		case *StringFilter_Contains:
			qb.AddLikeCondition("old_head_block", "%" + filter.Contains + "%")
		case *StringFilter_StartsWith:
			qb.AddLikeCondition("old_head_block", filter.StartsWith + "%")
		case *StringFilter_EndsWith:
			qb.AddLikeCondition("old_head_block", "%" + filter.EndsWith)
		case *StringFilter_Like:
			qb.AddLikeCondition("old_head_block", filter.Like)
		case *StringFilter_NotLike:
			qb.AddNotLikeCondition("old_head_block", filter.NotLike)
		case *StringFilter_In:
			if len(filter.In.Values) > 0 {
				qb.AddInCondition("old_head_block", StringSliceToInterface(filter.In.Values))
			}
		case *StringFilter_NotIn:
			if len(filter.NotIn.Values) > 0 {
				qb.AddNotInCondition("old_head_block", StringSliceToInterface(filter.NotIn.Values))
			}
		default:
			// Unsupported filter type
		}
	...
	// Add filter for column: new_head_block
	if req.NewHeadBlock != nil {
		switch filter := req.NewHeadBlock.Filter.(type) {
		case *StringFilter_Eq:
			qb.AddCondition("new_head_block", "=", filter.Eq)
		case *StringFilter_Ne:
			qb.AddCondition("new_head_block", "!=", filter.Ne)
		case *StringFilter_Contains:
			qb.AddLikeCondition("new_head_block", "%" + filter.Contains + "%")
		case *StringFilter_StartsWith:
			qb.AddLikeCondition("new_head_block", filter.StartsWith + "%")
		case *StringFilter_EndsWith:
			qb.AddLikeCondition("new_head_block", "%" + filter.EndsWith)
		case *StringFilter_Like:
			qb.AddLikeCondition("new_head_block", filter.Like)
		case *StringFilter_NotLike:
			qb.AddNotLikeCondition("new_head_block", filter.NotLike)
		case *StringFilter_In:
			if len(filter.In.Values) > 0 {
				qb.AddInCondition("new_head_block", StringSliceToInterface(filter.In.Values))
			}
		case *StringFilter_NotIn:
			if len(filter.NotIn.Values) > 0 {
				qb.AddNotInCondition("new_head_block", StringSliceToInterface(filter.NotIn.Values))
			}
		default:
			// Unsupported filter type
		}
	...
	// Add filter for column: old_head_state
	if req.OldHeadState != nil {
		switch filter := req.OldHeadState.Filter.(type) {
		case *StringFilter_Eq:
			qb.AddCondition("old_head_state", "=", filter.Eq)
		case *StringFilter_Ne:
			qb.AddCondition("old_head_state", "!=", filter.Ne)
		case *StringFilter_Contains:
			qb.AddLikeCondition("old_head_state", "%" + filter.Contains + "%")
		case *StringFilter_StartsWith:
			qb.AddLikeCondition("old_head_state", filter.StartsWith + "%")
		case *StringFilter_EndsWith:
			qb.AddLikeCondition("old_head_state", "%" + filter.EndsWith)
		case *StringFilter_Like:
			qb.AddLikeCondition("old_head_state", filter.Like)
		case *StringFilter_NotLike:
			qb.AddNotLikeCondition("old_head_state", filter.NotLike)
		case *StringFilter_In:
			if len(filter.In.Values) > 0 {
				qb.AddInCondition("old_head_state", StringSliceToInterface(filter.In.Values))
			}
		case *StringFilter_NotIn:
			if len(filter.NotIn.Values) > 0 {
				qb.AddNotInCondition("old_head_state", StringSliceToInterface(filter.NotIn.Values))
			}
		default:
			// Unsupported filter type
		}
	...
	// Add filter for column: new_head_state
	if req.NewHeadState != nil {
		switch filter := req.NewHeadState.Filter.(type) {
		case *StringFilter_Eq:
			qb.AddCondition("new_head_state", "=", filter.Eq)
		case *StringFilter_Ne:
			qb.AddCondition("new_head_state", "!=", filter.Ne)
		case *StringFilter_Contains:
			qb.AddLikeCondition("new_head_state", "%" + filter.Contains + "%")
		case *StringFilter_StartsWith:
			qb.AddLikeCondition("new_head_state", filter.StartsWith + "%")
		case *StringFilter_EndsWith:
			qb.AddLikeCondition("new_head_state", "%" + filter.EndsWith)
		case *StringFilter_Like:
			qb.AddLikeCondition("new_head_state", filter.Like)
		case *StringFilter_NotLike:
			qb.AddNotLikeCondition("new_head_state", filter.NotLike)
		case *StringFilter_In:
			if len(filter.In.Values) > 0 {
				qb.AddInCondition("new_head_state", StringSliceToInterface(filter.In.Values))
			}
		case *StringFilter_NotIn:
			if len(filter.NotIn.Values) > 0 {
				qb.AddNotInCondition("new_head_state", StringSliceToInterface(filter.NotIn.Values))
			}
		default:
			// Unsupported filter type
		}
	...
	// Add filter for column: execution_optimistic
	if req.ExecutionOptimistic != nil {
		switch filter := req.ExecutionOptimistic.Filter.(type) {
		case *BoolFilter_Eq:
			qb.AddCondition("execution_optimistic", "=", filter.Eq)
		case *BoolFilter_Ne:
			qb.AddCondition("execution_optimistic", "!=", filter.Ne)
		default:
			// Unsupported filter type
		}
	}
	...
	if req.OrderBy != "" {
		validFields := []string{"updated_date_time", "event_date_time", "slot", "slot_start_date_time", "propagation_slot_start_diff", "depth", "old_head_block", "new_head_block", "old_head_state", "new_head_state", "epoch", "epoch_start_date_time", "execution_optimistic", "meta_client_name", "meta_client_version", "meta_client_implementation", "meta_client_os", "meta_client_ip", "meta_client_geo_city", "meta_client_geo_country", "meta_client_geo_country_code", "meta_client_geo_continent_code", "meta_client_geo_longitude", "meta_client_geo_latitude", "meta_client_geo_autonomous_system_number", "meta_client_geo_autonomous_system_organization", "meta_network_name", "meta_consensus_version", "meta_consensus_version_major", "meta_consensus_version_minor", "meta_consensus_version_patch", "meta_consensus_implementation"}
		...
		orderByClause = " ORDER BY meta_network_name" + ", slot_start_date_time" + ", meta_client_name" + ", old_head_block" + ", new_head_block"
	}

	// Build column list
	columns := []string{"toUnixTimestamp(`updated_date_time`) AS `updated_date_time`", "toUnixTimestamp64Micro(`event_date_time`) AS `event_date_time`", "slot", "toUnixTimestamp(`slot_start_date_time`) AS `slot_start_date_time`", "propagation_slot_start_diff", "toUInt32(`depth`) AS `depth`", "NULLIF(`old_head_block`, repeat('\x00', 66)) AS `old_head_block`", "NULLIF(`new_head_block`, repeat('\x00', 66)) AS `new_head_block`", "NULLIF(`old_head_state`, repeat('\x00', 66)) AS `old_head_state`", "NULLIF(`new_head_state`, repeat('\x00', 66)) AS `new_head_state`", "epoch", "toUnixTimestamp(`epoch_start_date_time`) AS `epoch_start_date_time`", "execution_optimistic", "meta_client_name", "meta_client_version", "meta_client_implementation", "meta_client_os", "meta_client_ip", "meta_client_geo_city", "meta_client_geo_country", "meta_client_geo_country_code", "meta_client_geo_continent_code", "meta_client_geo_longitude", "meta_client_geo_latitude", "meta_client_geo_autonomous_system_number", "meta_client_geo_autonomous_system_organization", "meta_network_name", "meta_consensus_version", "meta_consensus_version_major", "meta_consensus_version_minor", "meta_consensus_version_patch", "meta_consensus_implementation"}

	return BuildParameterizedQuery("beacon_api_eth_v1_events_chain_reorg", columns, qb, orderByClause, limit, offset, options...)
}

// BuildGetBeaconApiEthV1EventsChainReorgQuery constructs a parameterized SQL query from a GetBeaconApiEthV1EventsChainReorgRequest
func BuildGetBeaconApiEthV1EventsChainReorgQuery(req *GetBeaconApiEthV1EventsChainReorgRequest, options ...QueryOption) (SQLQuery, error) {
	...
	return BuildParameterizedQuery("beacon_api_eth_v1_events_chain_reorg", columns, qb, orderByClause, 1, 0, options...)
}
