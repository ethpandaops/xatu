...
// SQL query builder for beacon_api_eth_v1_events_finalized_checkpoint
...
// BuildListBeaconApiEthV1EventsFinalizedCheckpointQuery constructs a parameterized SQL query from a ListBeaconApiEthV1EventsFinalizedCheckpointRequest
func BuildListBeaconApiEthV1EventsFinalizedCheckpointQuery(req *ListBeaconApiEthV1EventsFinalizedCheckpointRequest, options ...QueryOption) (SQLQuery, error) {
	...
	// Add filter for column: state
	if req.State != nil {
		switch filter := req.State.Filter.(type) {
		case *StringFilter_Eq:
			qb.AddCondition("state", "=", filter.Eq)
		case *StringFilter_Ne:
			qb.AddCondition("state", "!=", filter.Ne)
		case *StringFilter_Contains:
			qb.AddLikeCondition("state", "%" + filter.Contains + "%")
		case *StringFilter_StartsWith:
			qb.AddLikeCondition("state", filter.StartsWith + "%")
		case *StringFilter_EndsWith:
			qb.AddLikeCondition("state", "%" + filter.EndsWith)
		case *StringFilter_Like:
			qb.AddLikeCondition("state", filter.Like)
		case *StringFilter_NotLike:
			qb.AddNotLikeCondition("state", filter.NotLike)
		case *StringFilter_In:
			if len(filter.In.Values) > 0 {
				qb.AddInCondition("state", StringSliceToInterface(filter.In.Values))
			}
		case *StringFilter_NotIn:
			if len(filter.NotIn.Values) > 0 {
				qb.AddNotInCondition("state", StringSliceToInterface(filter.NotIn.Values))
			}
		default:
			// Unsupported filter type
		}
	...
	if req.OrderBy != "" {
		validFields := []string{"updated_date_time", "event_date_time", "block", "state", "epoch", "epoch_start_date_time", "execution_optimistic", "meta_client_name", "meta_client_version", "meta_client_implementation", "meta_client_os", "meta_client_ip", "meta_client_geo_city", "meta_client_geo_country", "meta_client_geo_country_code", "meta_client_geo_continent_code", "meta_client_geo_longitude", "meta_client_geo_latitude", "meta_client_geo_autonomous_system_number", "meta_client_geo_autonomous_system_organization", "meta_network_name", "meta_consensus_version", "meta_consensus_version_major", "meta_consensus_version_minor", "meta_consensus_version_patch", "meta_consensus_implementation"}
		...
		orderByClause = " ORDER BY meta_network_name" + ", epoch_start_date_time" + ", meta_client_name" + ", block" + ", state"
	}

	// Build column list
	columns := []string{"toUnixTimestamp(`updated_date_time`) AS `updated_date_time`", "toUnixTimestamp64Micro(`event_date_time`) AS `event_date_time`", "NULLIF(`block`, repeat('\x00', 66)) AS `block`", "NULLIF(`state`, repeat('\x00', 66)) AS `state`", "epoch", "toUnixTimestamp(`epoch_start_date_time`) AS `epoch_start_date_time`", "execution_optimistic", "meta_client_name", "meta_client_version", "meta_client_implementation", "meta_client_os", "meta_client_ip", "meta_client_geo_city", "meta_client_geo_country", "meta_client_geo_country_code", "meta_client_geo_continent_code", "meta_client_geo_longitude", "meta_client_geo_latitude", "meta_client_geo_autonomous_system_number", "meta_client_geo_autonomous_system_organization", "meta_network_name", "meta_consensus_version", "meta_consensus_version_major", "meta_consensus_version_minor", "meta_consensus_version_patch", "meta_consensus_implementation"}

	return BuildParameterizedQuery("beacon_api_eth_v1_events_finalized_checkpoint", columns, qb, orderByClause, limit, offset, options...)
}

// BuildGetBeaconApiEthV1EventsFinalizedCheckpointQuery constructs a parameterized SQL query from a GetBeaconApiEthV1EventsFinalizedCheckpointRequest
func BuildGetBeaconApiEthV1EventsFinalizedCheckpointQuery(req *GetBeaconApiEthV1EventsFinalizedCheckpointRequest, options ...QueryOption) (SQLQuery, error) {
	...
	return BuildParameterizedQuery("beacon_api_eth_v1_events_finalized_checkpoint", columns, qb, orderByClause, 1, 0, options...)
}
