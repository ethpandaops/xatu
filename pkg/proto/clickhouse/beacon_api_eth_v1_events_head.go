...
// SQL query builder for beacon_api_eth_v1_events_head
...
// BuildListBeaconApiEthV1EventsHeadQuery constructs a parameterized SQL query from a ListBeaconApiEthV1EventsHeadRequest
func BuildListBeaconApiEthV1EventsHeadQuery(req *ListBeaconApiEthV1EventsHeadRequest, options ...QueryOption) (SQLQuery, error) {
	...
	// Add filter for column: epoch_transition
	if req.EpochTransition != nil {
		switch filter := req.EpochTransition.Filter.(type) {
		case *BoolFilter_Eq:
			qb.AddCondition("epoch_transition", "=", filter.Eq)
		case *BoolFilter_Ne:
			qb.AddCondition("epoch_transition", "!=", filter.Ne)
		default:
			// Unsupported filter type
		}
	}
	...
	// Add filter for column: previous_duty_dependent_root
	if req.PreviousDutyDependentRoot != nil {
		switch filter := req.PreviousDutyDependentRoot.Filter.(type) {
		case *StringFilter_Eq:
			qb.AddCondition("previous_duty_dependent_root", "=", filter.Eq)
		case *StringFilter_Ne:
			qb.AddCondition("previous_duty_dependent_root", "!=", filter.Ne)
		case *StringFilter_Contains:
			qb.AddLikeCondition("previous_duty_dependent_root", "%" + filter.Contains + "%")
		case *StringFilter_StartsWith:
			qb.AddLikeCondition("previous_duty_dependent_root", filter.StartsWith + "%")
		case *StringFilter_EndsWith:
			qb.AddLikeCondition("previous_duty_dependent_root", "%" + filter.EndsWith)
		case *StringFilter_Like:
			qb.AddLikeCondition("previous_duty_dependent_root", filter.Like)
		case *StringFilter_NotLike:
			qb.AddNotLikeCondition("previous_duty_dependent_root", filter.NotLike)
		case *StringFilter_In:
			if len(filter.In.Values) > 0 {
				qb.AddInCondition("previous_duty_dependent_root", StringSliceToInterface(filter.In.Values))
			}
		case *StringFilter_NotIn:
			if len(filter.NotIn.Values) > 0 {
				qb.AddNotInCondition("previous_duty_dependent_root", StringSliceToInterface(filter.NotIn.Values))
			}
		default:
			// Unsupported filter type
		}
	...
	// Add filter for column: current_duty_dependent_root
	if req.CurrentDutyDependentRoot != nil {
		switch filter := req.CurrentDutyDependentRoot.Filter.(type) {
		case *StringFilter_Eq:
			qb.AddCondition("current_duty_dependent_root", "=", filter.Eq)
		case *StringFilter_Ne:
			qb.AddCondition("current_duty_dependent_root", "!=", filter.Ne)
		case *StringFilter_Contains:
			qb.AddLikeCondition("current_duty_dependent_root", "%" + filter.Contains + "%")
		case *StringFilter_StartsWith:
			qb.AddLikeCondition("current_duty_dependent_root", filter.StartsWith + "%")
		case *StringFilter_EndsWith:
			qb.AddLikeCondition("current_duty_dependent_root", "%" + filter.EndsWith)
		case *StringFilter_Like:
			qb.AddLikeCondition("current_duty_dependent_root", filter.Like)
		case *StringFilter_NotLike:
			qb.AddNotLikeCondition("current_duty_dependent_root", filter.NotLike)
		case *StringFilter_In:
			if len(filter.In.Values) > 0 {
				qb.AddInCondition("current_duty_dependent_root", StringSliceToInterface(filter.In.Values))
			}
		case *StringFilter_NotIn:
			if len(filter.NotIn.Values) > 0 {
				qb.AddNotInCondition("current_duty_dependent_root", StringSliceToInterface(filter.NotIn.Values))
			}
		default:
			// Unsupported filter type
		}
	...
	if req.OrderBy != "" {
		validFields := []string{"updated_date_time", "event_date_time", "slot", "slot_start_date_time", "propagation_slot_start_diff", "block", "epoch", "epoch_start_date_time", "epoch_transition", "execution_optimistic", "previous_duty_dependent_root", "current_duty_dependent_root", "meta_client_name", "meta_client_version", "meta_client_implementation", "meta_client_os", "meta_client_ip", "meta_client_geo_city", "meta_client_geo_country", "meta_client_geo_country_code", "meta_client_geo_continent_code", "meta_client_geo_longitude", "meta_client_geo_latitude", "meta_client_geo_autonomous_system_number", "meta_client_geo_autonomous_system_organization", "meta_network_name", "meta_consensus_version", "meta_consensus_version_major", "meta_consensus_version_minor", "meta_consensus_version_patch", "meta_consensus_implementation"}
		...
		orderByClause = " ORDER BY meta_network_name" + ", slot_start_date_time" + ", meta_client_name" + ", block" + ", previous_duty_dependent_root" + ", current_duty_dependent_root"
	}

	// Build column list
	columns := []string{"toUnixTimestamp(`updated_date_time`) AS `updated_date_time`", "toUnixTimestamp64Micro(`event_date_time`) AS `event_date_time`", "slot", "toUnixTimestamp(`slot_start_date_time`) AS `slot_start_date_time`", "propagation_slot_start_diff", "NULLIF(`block`, repeat('\x00', 66)) AS `block`", "epoch", "toUnixTimestamp(`epoch_start_date_time`) AS `epoch_start_date_time`", "epoch_transition", "execution_optimistic", "NULLIF(`previous_duty_dependent_root`, repeat('\x00', 66)) AS `previous_duty_dependent_root`", "NULLIF(`current_duty_dependent_root`, repeat('\x00', 66)) AS `current_duty_dependent_root`", "meta_client_name", "meta_client_version", "meta_client_implementation", "meta_client_os", "meta_client_ip", "meta_client_geo_city", "meta_client_geo_country", "meta_client_geo_country_code", "meta_client_geo_continent_code", "meta_client_geo_longitude", "meta_client_geo_latitude", "meta_client_geo_autonomous_system_number", "meta_client_geo_autonomous_system_organization", "meta_network_name", "meta_consensus_version", "meta_consensus_version_major", "meta_consensus_version_minor", "meta_consensus_version_patch", "meta_consensus_implementation"}

	return BuildParameterizedQuery("beacon_api_eth_v1_events_head", columns, qb, orderByClause, limit, offset, options...)
}

// BuildGetBeaconApiEthV1EventsHeadQuery constructs a parameterized SQL query from a GetBeaconApiEthV1EventsHeadRequest
func BuildGetBeaconApiEthV1EventsHeadQuery(req *GetBeaconApiEthV1EventsHeadRequest, options ...QueryOption) (SQLQuery, error) {
	...
	return BuildParameterizedQuery("beacon_api_eth_v1_events_head", columns, qb, orderByClause, 1, 0, options...)
}
