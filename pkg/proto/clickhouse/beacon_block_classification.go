...
// SQL query builder for beacon_block_classification
...
// BuildListBeaconBlockClassificationQuery constructs a parameterized SQL query from a ListBeaconBlockClassificationRequest
func BuildListBeaconBlockClassificationQuery(req *ListBeaconBlockClassificationRequest, options ...QueryOption) (SQLQuery, error) {
	...
	// Add filter for column: best_guess_single
	if req.BestGuessSingle != nil {
		switch filter := req.BestGuessSingle.Filter.(type) {
		case *StringFilter_Eq:
			qb.AddCondition("best_guess_single", "=", filter.Eq)
		case *StringFilter_Ne:
			qb.AddCondition("best_guess_single", "!=", filter.Ne)
		case *StringFilter_Contains:
			qb.AddLikeCondition("best_guess_single", "%" + filter.Contains + "%")
		case *StringFilter_StartsWith:
			qb.AddLikeCondition("best_guess_single", filter.StartsWith + "%")
		case *StringFilter_EndsWith:
			qb.AddLikeCondition("best_guess_single", "%" + filter.EndsWith)
		case *StringFilter_Like:
			qb.AddLikeCondition("best_guess_single", filter.Like)
		case *StringFilter_NotLike:
			qb.AddNotLikeCondition("best_guess_single", filter.NotLike)
		case *StringFilter_In:
			if len(filter.In.Values) > 0 {
				qb.AddInCondition("best_guess_single", StringSliceToInterface(filter.In.Values))
			}
		case *StringFilter_NotIn:
			if len(filter.NotIn.Values) > 0 {
				qb.AddNotInCondition("best_guess_single", StringSliceToInterface(filter.NotIn.Values))
			}
		default:
			// Unsupported filter type
		}
	...
	// Add filter for column: best_guess_multi
	if req.BestGuessMulti != nil {
		switch filter := req.BestGuessMulti.Filter.(type) {
		case *StringFilter_Eq:
			qb.AddCondition("best_guess_multi", "=", filter.Eq)
		case *StringFilter_Ne:
			qb.AddCondition("best_guess_multi", "!=", filter.Ne)
		case *StringFilter_Contains:
			qb.AddLikeCondition("best_guess_multi", "%" + filter.Contains + "%")
		case *StringFilter_StartsWith:
			qb.AddLikeCondition("best_guess_multi", filter.StartsWith + "%")
		case *StringFilter_EndsWith:
			qb.AddLikeCondition("best_guess_multi", "%" + filter.EndsWith)
		case *StringFilter_Like:
			qb.AddLikeCondition("best_guess_multi", filter.Like)
		case *StringFilter_NotLike:
			qb.AddNotLikeCondition("best_guess_multi", filter.NotLike)
		case *StringFilter_In:
			if len(filter.In.Values) > 0 {
				qb.AddInCondition("best_guess_multi", StringSliceToInterface(filter.In.Values))
			}
		case *StringFilter_NotIn:
			if len(filter.NotIn.Values) > 0 {
				qb.AddNotInCondition("best_guess_multi", StringSliceToInterface(filter.NotIn.Values))
			}
		default:
			// Unsupported filter type
		}
	...
	// Add filter for column: client_probability_uncertain

	// Add filter for column: client_probability_prysm

	// Add filter for column: client_probability_teku

	// Add filter for column: client_probability_nimbus

	// Add filter for column: client_probability_lodestar

	// Add filter for column: client_probability_grandine

	// Add filter for column: client_probability_lighthouse
	...
	if req.OrderBy != "" {
		...
		orderByClause = " ORDER BY meta_network_name" + ", slot_start_date_time" + ", meta_client_name" + ", proposer_index"
	}

	// Build column list
	columns := []string{"toUnixTimestamp(`updated_date_time`) AS `updated_date_time`", "toUnixTimestamp64Micro(`event_date_time`) AS `event_date_time`", "slot", "toUnixTimestamp(`slot_start_date_time`) AS `slot_start_date_time`", "epoch", "toUnixTimestamp(`epoch_start_date_time`) AS `epoch_start_date_time`", "best_guess_single", "best_guess_multi", "client_probability_uncertain", "client_probability_prysm", "client_probability_teku", "client_probability_nimbus", "client_probability_lodestar", "client_probability_grandine", "client_probability_lighthouse", "proposer_index", "meta_client_name", "meta_client_version", "meta_client_implementation", "meta_client_os", "meta_client_ip", "meta_client_geo_city", "meta_client_geo_country", "meta_client_geo_country_code", "meta_client_geo_continent_code", "meta_client_geo_longitude", "meta_client_geo_latitude", "meta_client_geo_autonomous_system_number", "meta_client_geo_autonomous_system_organization", "meta_network_name", "meta_consensus_version", "meta_consensus_version_major", "meta_consensus_version_minor", "meta_consensus_version_patch", "meta_consensus_implementation"}

	return BuildParameterizedQuery("beacon_block_classification", columns, qb, orderByClause, limit, offset, options...)
}

// BuildGetBeaconBlockClassificationQuery constructs a parameterized SQL query from a GetBeaconBlockClassificationRequest
func BuildGetBeaconBlockClassificationQuery(req *GetBeaconBlockClassificationRequest, options ...QueryOption) (SQLQuery, error) {
	...
	return BuildParameterizedQuery("beacon_block_classification", columns, qb, orderByClause, 1, 0, options...)
}
