...
// SQL query builder for canonical_beacon_block_execution_request_withdrawal
...
// BuildListCanonicalBeaconBlockExecutionRequestWithdrawalQuery constructs a parameterized SQL query from a ListCanonicalBeaconBlockExecutionRequestWithdrawalRequest
func BuildListCanonicalBeaconBlockExecutionRequestWithdrawalQuery(req *ListCanonicalBeaconBlockExecutionRequestWithdrawalRequest, options ...QueryOption) (SQLQuery, error) {
	...
	// Add filter for column: validator_pubkey
	if req.ValidatorPubkey != nil {
		switch filter := req.ValidatorPubkey.Filter.(type) {
		case *StringFilter_Eq:
			qb.AddCondition("validator_pubkey", "=", filter.Eq)
		case *StringFilter_Ne:
			qb.AddCondition("validator_pubkey", "!=", filter.Ne)
		case *StringFilter_Contains:
			qb.AddLikeCondition("validator_pubkey", "%" + filter.Contains + "%")
		case *StringFilter_StartsWith:
			qb.AddLikeCondition("validator_pubkey", filter.StartsWith + "%")
		case *StringFilter_EndsWith:
			qb.AddLikeCondition("validator_pubkey", "%" + filter.EndsWith)
		case *StringFilter_Like:
			qb.AddLikeCondition("validator_pubkey", filter.Like)
		case *StringFilter_NotLike:
			qb.AddNotLikeCondition("validator_pubkey", filter.NotLike)
		case *StringFilter_In:
			if len(filter.In.Values) > 0 {
				qb.AddInCondition("validator_pubkey", StringSliceToInterface(filter.In.Values))
			}
		case *StringFilter_NotIn:
			if len(filter.NotIn.Values) > 0 {
				qb.AddNotInCondition("validator_pubkey", StringSliceToInterface(filter.NotIn.Values))
			}
		default:
			// Unsupported filter type
		}
	...
	if req.OrderBy != "" {
		validFields := []string{"updated_date_time", "slot", "slot_start_date_time", "epoch", "epoch_start_date_time", "block_root", "block_version", "position_in_block", "source_address", "validator_pubkey", "amount", "meta_network_name"}
	...
	// Build column list
	columns := []string{"toUnixTimestamp(`updated_date_time`) AS `updated_date_time`", "slot", "toUnixTimestamp(`slot_start_date_time`) AS `slot_start_date_time`", "epoch", "toUnixTimestamp(`epoch_start_date_time`) AS `epoch_start_date_time`", "NULLIF(`block_root`, repeat('\x00', 66)) AS `block_root`", "block_version", "position_in_block", "NULLIF(`source_address`, repeat('\x00', 42)) AS `source_address`", "validator_pubkey", "toString(`amount`) AS `amount`", "meta_network_name"}

	return BuildParameterizedQuery("canonical_beacon_block_execution_request_withdrawal", columns, qb, orderByClause, limit, offset, options...)
}

// BuildGetCanonicalBeaconBlockExecutionRequestWithdrawalQuery constructs a parameterized SQL query from a GetCanonicalBeaconBlockExecutionRequestWithdrawalRequest
func BuildGetCanonicalBeaconBlockExecutionRequestWithdrawalQuery(req *GetCanonicalBeaconBlockExecutionRequestWithdrawalRequest, options ...QueryOption) (SQLQuery, error) {
	...
	return BuildParameterizedQuery("canonical_beacon_block_execution_request_withdrawal", columns, qb, orderByClause, 1, 0, options...)
}
