...
// SQL query builder for canonical_execution_address_appearances
...
// BuildListCanonicalExecutionAddressAppearancesQuery constructs a parameterized SQL query from a ListCanonicalExecutionAddressAppearancesRequest
func BuildListCanonicalExecutionAddressAppearancesQuery(req *ListCanonicalExecutionAddressAppearancesRequest, options ...QueryOption) (SQLQuery, error) {
	...
	// Add filter for column: relationship
	if req.Relationship != nil {
		switch filter := req.Relationship.Filter.(type) {
		case *StringFilter_Eq:
			qb.AddCondition("relationship", "=", filter.Eq)
		case *StringFilter_Ne:
			qb.AddCondition("relationship", "!=", filter.Ne)
		case *StringFilter_Contains:
			qb.AddLikeCondition("relationship", "%" + filter.Contains + "%")
		case *StringFilter_StartsWith:
			qb.AddLikeCondition("relationship", filter.StartsWith + "%")
		case *StringFilter_EndsWith:
			qb.AddLikeCondition("relationship", "%" + filter.EndsWith)
		case *StringFilter_Like:
			qb.AddLikeCondition("relationship", filter.Like)
		case *StringFilter_NotLike:
			qb.AddNotLikeCondition("relationship", filter.NotLike)
		case *StringFilter_In:
			if len(filter.In.Values) > 0 {
				qb.AddInCondition("relationship", StringSliceToInterface(filter.In.Values))
			}
		case *StringFilter_NotIn:
			if len(filter.NotIn.Values) > 0 {
				qb.AddNotInCondition("relationship", StringSliceToInterface(filter.NotIn.Values))
			}
		default:
			// Unsupported filter type
		}
	...
	if req.OrderBy != "" {
		validFields := []string{"updated_date_time", "block_number", "transaction_hash", "internal_index", "address", "relationship", "meta_network_name"}
	...
	// Build column list
	columns := []string{"toUnixTimestamp(`updated_date_time`) AS `updated_date_time`", "block_number", "NULLIF(`transaction_hash`, repeat('\x00', 66)) AS `transaction_hash`", "internal_index", "address", "relationship", "meta_network_name"}

	return BuildParameterizedQuery("canonical_execution_address_appearances", columns, qb, orderByClause, limit, offset, options...)
}

// BuildGetCanonicalExecutionAddressAppearancesQuery constructs a parameterized SQL query from a GetCanonicalExecutionAddressAppearancesRequest
func BuildGetCanonicalExecutionAddressAppearancesQuery(req *GetCanonicalExecutionAddressAppearancesRequest, options ...QueryOption) (SQLQuery, error) {
	...
	return BuildParameterizedQuery("canonical_execution_address_appearances", columns, qb, orderByClause, 1, 0, options...)
}
