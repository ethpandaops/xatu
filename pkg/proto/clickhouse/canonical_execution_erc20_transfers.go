...
// SQL query builder for canonical_execution_erc20_transfers
...
// BuildListCanonicalExecutionErc20TransfersQuery constructs a parameterized SQL query from a ListCanonicalExecutionErc20TransfersRequest
func BuildListCanonicalExecutionErc20TransfersQuery(req *ListCanonicalExecutionErc20TransfersRequest, options ...QueryOption) (SQLQuery, error) {
	...
	// Add filter for column: erc20
	if req.Erc20 != nil {
		switch filter := req.Erc20.Filter.(type) {
		case *StringFilter_Eq:
			qb.AddCondition("erc20", "=", filter.Eq)
		case *StringFilter_Ne:
			qb.AddCondition("erc20", "!=", filter.Ne)
		case *StringFilter_Contains:
			qb.AddLikeCondition("erc20", "%" + filter.Contains + "%")
		case *StringFilter_StartsWith:
			qb.AddLikeCondition("erc20", filter.StartsWith + "%")
		case *StringFilter_EndsWith:
			qb.AddLikeCondition("erc20", "%" + filter.EndsWith)
		case *StringFilter_Like:
			qb.AddLikeCondition("erc20", filter.Like)
		case *StringFilter_NotLike:
			qb.AddNotLikeCondition("erc20", filter.NotLike)
		case *StringFilter_In:
			if len(filter.In.Values) > 0 {
				qb.AddInCondition("erc20", StringSliceToInterface(filter.In.Values))
			}
		case *StringFilter_NotIn:
			if len(filter.NotIn.Values) > 0 {
				qb.AddNotInCondition("erc20", StringSliceToInterface(filter.NotIn.Values))
			}
		default:
			// Unsupported filter type
		}
	...
	if req.OrderBy != "" {
		validFields := []string{"updated_date_time", "block_number", "transaction_index", "transaction_hash", "internal_index", "log_index", "erc20", "from_address", "to_address", "value", "meta_network_name"}
	...
	// Build column list
	columns := []string{"toUnixTimestamp(`updated_date_time`) AS `updated_date_time`", "block_number", "transaction_index", "NULLIF(`transaction_hash`, repeat('\x00', 66)) AS `transaction_hash`", "internal_index", "log_index", "erc20", "from_address", "to_address", "toString(`value`) AS `value`", "meta_network_name"}

	return BuildParameterizedQuery("canonical_execution_erc20_transfers", columns, qb, orderByClause, limit, offset, options...)
}

// BuildGetCanonicalExecutionErc20TransfersQuery constructs a parameterized SQL query from a GetCanonicalExecutionErc20TransfersRequest
func BuildGetCanonicalExecutionErc20TransfersQuery(req *GetCanonicalExecutionErc20TransfersRequest, options ...QueryOption) (SQLQuery, error) {
	...
	return BuildParameterizedQuery("canonical_execution_erc20_transfers", columns, qb, orderByClause, 1, 0, options...)
}
