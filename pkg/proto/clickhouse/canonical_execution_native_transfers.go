...
// SQL query builder for canonical_execution_native_transfers
...
// BuildListCanonicalExecutionNativeTransfersQuery constructs a parameterized SQL query from a ListCanonicalExecutionNativeTransfersRequest
func BuildListCanonicalExecutionNativeTransfersQuery(req *ListCanonicalExecutionNativeTransfersRequest, options ...QueryOption) (SQLQuery, error) {
	...
	// Add filter for column: transfer_index
	if req.TransferIndex != nil {
		switch filter := req.TransferIndex.Filter.(type) {
		case *UInt64Filter_Eq:
			qb.AddCondition("transfer_index", "=", filter.Eq)
		case *UInt64Filter_Ne:
			qb.AddCondition("transfer_index", "!=", filter.Ne)
		case *UInt64Filter_Lt:
			qb.AddCondition("transfer_index", "<", filter.Lt)
		case *UInt64Filter_Lte:
			qb.AddCondition("transfer_index", "<=", filter.Lte)
		case *UInt64Filter_Gt:
			qb.AddCondition("transfer_index", ">", filter.Gt)
		case *UInt64Filter_Gte:
			qb.AddCondition("transfer_index", ">=", filter.Gte)
		case *UInt64Filter_Between:
			qb.AddBetweenCondition("transfer_index", filter.Between.Min, filter.Between.Max.GetValue())
		case *UInt64Filter_In:
			if len(filter.In.Values) > 0 {
				qb.AddInCondition("transfer_index", UInt64SliceToInterface(filter.In.Values))
			}
		case *UInt64Filter_NotIn:
			if len(filter.NotIn.Values) > 0 {
				qb.AddNotInCondition("transfer_index", UInt64SliceToInterface(filter.NotIn.Values))
			}
		default:
			// Unsupported filter type
		}
	...
	if req.OrderBy != "" {
		validFields := []string{"updated_date_time", "block_number", "transaction_index", "transaction_hash", "internal_index", "transfer_index", "from_address", "to_address", "value", "meta_network_name"}
	...
	// Build column list
	columns := []string{"toUnixTimestamp(`updated_date_time`) AS `updated_date_time`", "block_number", "transaction_index", "NULLIF(`transaction_hash`, repeat('\x00', 66)) AS `transaction_hash`", "internal_index", "transfer_index", "from_address", "to_address", "toString(`value`) AS `value`", "meta_network_name"}

	return BuildParameterizedQuery("canonical_execution_native_transfers", columns, qb, orderByClause, limit, offset, options...)
}

// BuildGetCanonicalExecutionNativeTransfersQuery constructs a parameterized SQL query from a GetCanonicalExecutionNativeTransfersRequest
func BuildGetCanonicalExecutionNativeTransfersQuery(req *GetCanonicalExecutionNativeTransfersRequest, options ...QueryOption) (SQLQuery, error) {
	...
	return BuildParameterizedQuery("canonical_execution_native_transfers", columns, qb, orderByClause, 1, 0, options...)
}
