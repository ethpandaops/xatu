...
// SQL query builder for canonical_execution_nonce_diffs
...
// BuildListCanonicalExecutionNonceDiffsQuery constructs a parameterized SQL query from a ListCanonicalExecutionNonceDiffsRequest
func BuildListCanonicalExecutionNonceDiffsQuery(req *ListCanonicalExecutionNonceDiffsRequest, options ...QueryOption) (SQLQuery, error) {
	...
	if req.FromValue != nil {
		...
		case *UInt64Filter_Lt:
			qb.AddCondition("from_value", "<", filter.Lt)
		case *UInt64Filter_Lte:
			qb.AddCondition("from_value", "<=", filter.Lte)
		case *UInt64Filter_Gt:
			qb.AddCondition("from_value", ">", filter.Gt)
		case *UInt64Filter_Gte:
			qb.AddCondition("from_value", ">=", filter.Gte)
		case *UInt64Filter_Between:
			qb.AddBetweenCondition("from_value", filter.Between.Min, filter.Between.Max.GetValue())
		case *UInt64Filter_In:
			if len(filter.In.Values) > 0 {
				qb.AddInCondition("from_value", UInt64SliceToInterface(filter.In.Values))
			}
		case *UInt64Filter_NotIn:
			if len(filter.NotIn.Values) > 0 {
				qb.AddNotInCondition("from_value", UInt64SliceToInterface(filter.NotIn.Values))
			}
		default:
			// Unsupported filter type
		}
	...
	if req.ToValue != nil {
		...
		case *UInt64Filter_Lt:
			qb.AddCondition("to_value", "<", filter.Lt)
		case *UInt64Filter_Lte:
			qb.AddCondition("to_value", "<=", filter.Lte)
		case *UInt64Filter_Gt:
			qb.AddCondition("to_value", ">", filter.Gt)
		case *UInt64Filter_Gte:
			qb.AddCondition("to_value", ">=", filter.Gte)
		case *UInt64Filter_Between:
			qb.AddBetweenCondition("to_value", filter.Between.Min, filter.Between.Max.GetValue())
		case *UInt64Filter_In:
			if len(filter.In.Values) > 0 {
				qb.AddInCondition("to_value", UInt64SliceToInterface(filter.In.Values))
			}
		case *UInt64Filter_NotIn:
			if len(filter.NotIn.Values) > 0 {
				qb.AddNotInCondition("to_value", UInt64SliceToInterface(filter.NotIn.Values))
			}
		default:
			// Unsupported filter type
		}
	...
	// Build column list
	columns := []string{"toUnixTimestamp(`updated_date_time`) AS `updated_date_time`", "block_number", "transaction_index", "NULLIF(`transaction_hash`, repeat('\x00', 66)) AS `transaction_hash`", "internal_index", "address", "from_value", "to_value", "meta_network_name"}

	return BuildParameterizedQuery("canonical_execution_nonce_diffs", columns, qb, orderByClause, limit, offset, options...)
}

// BuildGetCanonicalExecutionNonceDiffsQuery constructs a parameterized SQL query from a GetCanonicalExecutionNonceDiffsRequest
func BuildGetCanonicalExecutionNonceDiffsQuery(req *GetCanonicalExecutionNonceDiffsRequest, options ...QueryOption) (SQLQuery, error) {
	...
	return BuildParameterizedQuery("canonical_execution_nonce_diffs", columns, qb, orderByClause, 1, 0, options...)
}
