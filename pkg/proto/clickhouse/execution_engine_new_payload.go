...
// SQL query builder for execution_engine_new_payload
...
// BuildListExecutionEngineNewPayloadQuery constructs a parameterized SQL query from a ListExecutionEngineNewPayloadRequest
func BuildListExecutionEngineNewPayloadQuery(req *ListExecutionEngineNewPayloadRequest, options ...QueryOption) (SQLQuery, error) {
	...
	if req.OrderBy != "" {
		...
		orderByClause = " ORDER BY meta_network_name" + ", block_number" + ", meta_client_name" + ", block_hash" + ", event_date_time"
	}

	// Build column list
	columns := []string{"toUnixTimestamp(`updated_date_time`) AS `updated_date_time`", "toUnixTimestamp64Micro(`event_date_time`) AS `event_date_time`", "toUnixTimestamp64Micro(`requested_date_time`) AS `requested_date_time`", "duration_ms", "source", "block_number", "NULLIF(`block_hash`, repeat('\x00', 66)) AS `block_hash`", "NULLIF(`parent_hash`, repeat('\x00', 66)) AS `parent_hash`", "gas_used", "gas_limit", "tx_count", "blob_count", "status", "NULLIF(`latest_valid_hash`, repeat('\x00', 66)) AS `latest_valid_hash`", "validation_error", "method_version", "meta_execution_implementation", "meta_execution_version", "meta_execution_version_major", "meta_execution_version_minor", "meta_execution_version_patch", "meta_client_name", "meta_client_version", "meta_client_implementation", "meta_client_os", "meta_client_ip", "meta_client_geo_city", "meta_client_geo_country", "meta_client_geo_country_code", "meta_client_geo_continent_code", "meta_client_geo_longitude", "meta_client_geo_latitude", "meta_client_geo_autonomous_system_number", "meta_client_geo_autonomous_system_organization", "meta_network_name"}

	return BuildParameterizedQuery("execution_engine_new_payload", columns, qb, orderByClause, limit, offset, options...)
}

// BuildGetExecutionEngineNewPayloadQuery constructs a parameterized SQL query from a GetExecutionEngineNewPayloadRequest
func BuildGetExecutionEngineNewPayloadQuery(req *GetExecutionEngineNewPayloadRequest, options ...QueryOption) (SQLQuery, error) {
	...
	return BuildParameterizedQuery("execution_engine_new_payload", columns, qb, orderByClause, 1, 0, options...)
}
