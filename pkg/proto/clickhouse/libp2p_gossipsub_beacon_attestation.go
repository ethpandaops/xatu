...
// SQL query builder for libp2p_gossipsub_beacon_attestation
...
// BuildListLibp2PGossipsubBeaconAttestationQuery constructs a parameterized SQL query from a ListLibp2PGossipsubBeaconAttestationRequest
func BuildListLibp2PGossipsubBeaconAttestationQuery(req *ListLibp2PGossipsubBeaconAttestationRequest, options ...QueryOption) (SQLQuery, error) {
	...
	if req.AttestingValidatorIndex != nil {
		...
		case *NullableUInt32Filter_IsNull:
			qb.AddIsNullCondition("attesting_validator_index")
		case *NullableUInt32Filter_IsNotNull:
			qb.AddIsNotNullCondition("attesting_validator_index")
		default:
			// Unsupported filter type
		}
	}

	// Add filter for column: attesting_validator_committee_index
	if req.AttestingValidatorCommitteeIndex != nil {
		switch filter := req.AttestingValidatorCommitteeIndex.Filter.(type) {
		case *StringFilter_Eq:
			qb.AddCondition("attesting_validator_committee_index", "=", filter.Eq)
		case *StringFilter_Ne:
			qb.AddCondition("attesting_validator_committee_index", "!=", filter.Ne)
		case *StringFilter_Contains:
			qb.AddLikeCondition("attesting_validator_committee_index", "%" + filter.Contains + "%")
		case *StringFilter_StartsWith:
			qb.AddLikeCondition("attesting_validator_committee_index", filter.StartsWith + "%")
		case *StringFilter_EndsWith:
			qb.AddLikeCondition("attesting_validator_committee_index", "%" + filter.EndsWith)
		case *StringFilter_Like:
			qb.AddLikeCondition("attesting_validator_committee_index", filter.Like)
		case *StringFilter_NotLike:
			qb.AddNotLikeCondition("attesting_validator_committee_index", filter.NotLike)
		case *StringFilter_In:
			if len(filter.In.Values) > 0 {
				qb.AddInCondition("attesting_validator_committee_index", StringSliceToInterface(filter.In.Values))
			}
		case *StringFilter_NotIn:
			if len(filter.NotIn.Values) > 0 {
				qb.AddNotInCondition("attesting_validator_committee_index", StringSliceToInterface(filter.NotIn.Values))
			}
		default:
			// Unsupported filter type
		}
	...
	// Build column list
	columns := []string{"toUnixTimestamp(`updated_date_time`) AS `updated_date_time`", "toUnixTimestamp64Micro(`event_date_time`) AS `event_date_time`", "slot", "toUnixTimestamp(`slot_start_date_time`) AS `slot_start_date_time`", "epoch", "toUnixTimestamp(`epoch_start_date_time`) AS `epoch_start_date_time`", "committee_index", "attesting_validator_index", "attesting_validator_committee_index", "wallclock_slot", "toUnixTimestamp(`wallclock_slot_start_date_time`) AS `wallclock_slot_start_date_time`", "wallclock_epoch", "toUnixTimestamp(`wallclock_epoch_start_date_time`) AS `wallclock_epoch_start_date_time`", "propagation_slot_start_diff", "peer_id_unique_key", "message_id", "message_size", "topic_layer", "topic_fork_digest_value", "topic_name", "topic_encoding", "aggregation_bits", "NULLIF(`beacon_block_root`, repeat('\x00', 66)) AS `beacon_block_root`", "source_epoch", "toUnixTimestamp(`source_epoch_start_date_time`) AS `source_epoch_start_date_time`", "NULLIF(`source_root`, repeat('\x00', 66)) AS `source_root`", "target_epoch", "toUnixTimestamp(`target_epoch_start_date_time`) AS `target_epoch_start_date_time`", "NULLIF(`target_root`, repeat('\x00', 66)) AS `target_root`", "meta_client_name", "meta_client_version", "meta_client_implementation", "meta_client_os", "meta_client_ip", "meta_client_geo_city", "meta_client_geo_country", "meta_client_geo_country_code", "meta_client_geo_continent_code", "meta_client_geo_longitude", "meta_client_geo_latitude", "meta_client_geo_autonomous_system_number", "meta_client_geo_autonomous_system_organization", "meta_network_name"}

	return BuildParameterizedQuery("libp2p_gossipsub_beacon_attestation", columns, qb, orderByClause, limit, offset, options...)
}

// BuildGetLibp2PGossipsubBeaconAttestationQuery constructs a parameterized SQL query from a GetLibp2PGossipsubBeaconAttestationRequest
func BuildGetLibp2PGossipsubBeaconAttestationQuery(req *GetLibp2PGossipsubBeaconAttestationRequest, options ...QueryOption) (SQLQuery, error) {
	...
	return BuildParameterizedQuery("libp2p_gossipsub_beacon_attestation", columns, qb, orderByClause, 1, 0, options...)
}
