...
// SQL query builder for libp2p_gossipsub_blob_sidecar
...
// BuildListLibp2PGossipsubBlobSidecarQuery constructs a parameterized SQL query from a ListLibp2PGossipsubBlobSidecarRequest
func BuildListLibp2PGossipsubBlobSidecarQuery(req *ListLibp2PGossipsubBlobSidecarRequest, options ...QueryOption) (SQLQuery, error) {
	...
	if req.BlobIndex != nil {
		...
		case *UInt32Filter_NotIn:
			if len(filter.NotIn.Values) > 0 {
				qb.AddNotInCondition("blob_index", UInt32SliceToInterface(filter.NotIn.Values))
			}
		default:
			// Unsupported filter type
		}
	...
	// Build column list
	columns := []string{"toUnixTimestamp(`updated_date_time`) AS `updated_date_time`", "toUnixTimestamp64Micro(`event_date_time`) AS `event_date_time`", "slot", "toUnixTimestamp(`slot_start_date_time`) AS `slot_start_date_time`", "epoch", "toUnixTimestamp(`epoch_start_date_time`) AS `epoch_start_date_time`", "wallclock_slot", "toUnixTimestamp(`wallclock_slot_start_date_time`) AS `wallclock_slot_start_date_time`", "wallclock_epoch", "toUnixTimestamp(`wallclock_epoch_start_date_time`) AS `wallclock_epoch_start_date_time`", "propagation_slot_start_diff", "proposer_index", "blob_index", "NULLIF(`beacon_block_root`, repeat('\x00', 66)) AS `beacon_block_root`", "NULLIF(`parent_root`, repeat('\x00', 66)) AS `parent_root`", "NULLIF(`state_root`, repeat('\x00', 66)) AS `state_root`", "peer_id_unique_key", "message_id", "message_size", "topic_layer", "topic_fork_digest_value", "topic_name", "topic_encoding", "meta_client_name", "meta_client_version", "meta_client_implementation", "meta_client_os", "meta_client_ip", "meta_client_geo_city", "meta_client_geo_country", "meta_client_geo_country_code", "meta_client_geo_continent_code", "meta_client_geo_longitude", "meta_client_geo_latitude", "meta_client_geo_autonomous_system_number", "meta_client_geo_autonomous_system_organization", "meta_network_name"}

	return BuildParameterizedQuery("libp2p_gossipsub_blob_sidecar", columns, qb, orderByClause, limit, offset, options...)
}

// BuildGetLibp2PGossipsubBlobSidecarQuery constructs a parameterized SQL query from a GetLibp2PGossipsubBlobSidecarRequest
func BuildGetLibp2PGossipsubBlobSidecarQuery(req *GetLibp2PGossipsubBlobSidecarRequest, options ...QueryOption) (SQLQuery, error) {
	...
	return BuildParameterizedQuery("libp2p_gossipsub_blob_sidecar", columns, qb, orderByClause, 1, 0, options...)
}
