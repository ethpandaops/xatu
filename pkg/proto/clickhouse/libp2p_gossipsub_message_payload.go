...
// SQL query builder for libp2p_gossipsub_message_payload
...
// BuildListLibp2PGossipsubMessagePayloadQuery constructs a parameterized SQL query from a ListLibp2PGossipsubMessagePayloadRequest
func BuildListLibp2PGossipsubMessagePayloadQuery(req *ListLibp2PGossipsubMessagePayloadRequest, options ...QueryOption) (SQLQuery, error) {
	...
	// Add filter for column: message_data
	if req.MessageData != nil {
		switch filter := req.MessageData.Filter.(type) {
		case *StringFilter_Eq:
			qb.AddCondition("message_data", "=", filter.Eq)
		case *StringFilter_Ne:
			qb.AddCondition("message_data", "!=", filter.Ne)
		case *StringFilter_Contains:
			qb.AddLikeCondition("message_data", "%" + filter.Contains + "%")
		case *StringFilter_StartsWith:
			qb.AddLikeCondition("message_data", filter.StartsWith + "%")
		case *StringFilter_EndsWith:
			qb.AddLikeCondition("message_data", "%" + filter.EndsWith)
		case *StringFilter_Like:
			qb.AddLikeCondition("message_data", filter.Like)
		case *StringFilter_NotLike:
			qb.AddNotLikeCondition("message_data", filter.NotLike)
		case *StringFilter_In:
			if len(filter.In.Values) > 0 {
				qb.AddInCondition("message_data", StringSliceToInterface(filter.In.Values))
			}
		case *StringFilter_NotIn:
			if len(filter.NotIn.Values) > 0 {
				qb.AddNotInCondition("message_data", StringSliceToInterface(filter.NotIn.Values))
			}
		default:
			// Unsupported filter type
		}
	...
	if req.OrderBy != "" {
		validFields := []string{"updated_date_time", "event_date_time", "wallclock_slot", "wallclock_slot_start_date_time", "wallclock_epoch", "wallclock_epoch_start_date_time", "topic_layer", "topic_fork_digest_value", "topic_name", "topic_encoding", "message_id", "message_size", "message_data", "meta_client_name", "meta_client_version", "meta_client_implementation", "meta_client_os", "meta_client_ip", "meta_client_geo_city", "meta_client_geo_country", "meta_client_geo_country_code", "meta_client_geo_continent_code", "meta_client_geo_longitude", "meta_client_geo_latitude", "meta_client_geo_autonomous_system_number", "meta_client_geo_autonomous_system_organization", "meta_network_name"}
		...
		orderByClause = " ORDER BY meta_network_name" + ", wallclock_slot_start_date_time" + ", topic_fork_digest_value" + ", topic_name" + ", message_id"
	}

	// Build column list
	columns := []string{"toUnixTimestamp(`updated_date_time`) AS `updated_date_time`", "toUnixTimestamp64Micro(`event_date_time`) AS `event_date_time`", "wallclock_slot", "toUnixTimestamp(`wallclock_slot_start_date_time`) AS `wallclock_slot_start_date_time`", "wallclock_epoch", "toUnixTimestamp(`wallclock_epoch_start_date_time`) AS `wallclock_epoch_start_date_time`", "topic_layer", "topic_fork_digest_value", "topic_name", "topic_encoding", "message_id", "message_size", "message_data", "meta_client_name", "meta_client_version", "meta_client_implementation", "meta_client_os", "meta_client_ip", "meta_client_geo_city", "meta_client_geo_country", "meta_client_geo_country_code", "meta_client_geo_continent_code", "meta_client_geo_longitude", "meta_client_geo_latitude", "meta_client_geo_autonomous_system_number", "meta_client_geo_autonomous_system_organization", "meta_network_name"}

	return BuildParameterizedQuery("libp2p_gossipsub_message_payload", columns, qb, orderByClause, limit, offset, options...)
}

// BuildGetLibp2PGossipsubMessagePayloadQuery constructs a parameterized SQL query from a GetLibp2PGossipsubMessagePayloadRequest
func BuildGetLibp2PGossipsubMessagePayloadQuery(req *GetLibp2PGossipsubMessagePayloadRequest, options ...QueryOption) (SQLQuery, error) {
	...
	return BuildParameterizedQuery("libp2p_gossipsub_message_payload", columns, qb, orderByClause, 1, 0, options...)
}
