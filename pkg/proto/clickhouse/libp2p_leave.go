...
// SQL query builder for libp2p_leave
...
// BuildListLibp2PLeaveQuery constructs a parameterized SQL query from a ListLibp2PLeaveRequest
func BuildListLibp2PLeaveQuery(req *ListLibp2PLeaveRequest, options ...QueryOption) (SQLQuery, error) {
	...
	// Add filter for column: local_peer_id_unique_key
	if req.LocalPeerIdUniqueKey != nil {
		switch filter := req.LocalPeerIdUniqueKey.Filter.(type) {
		case *Int64Filter_Eq:
			qb.AddCondition("local_peer_id_unique_key", "=", filter.Eq)
		case *Int64Filter_Ne:
			qb.AddCondition("local_peer_id_unique_key", "!=", filter.Ne)
		case *Int64Filter_Lt:
			qb.AddCondition("local_peer_id_unique_key", "<", filter.Lt)
		case *Int64Filter_Lte:
			qb.AddCondition("local_peer_id_unique_key", "<=", filter.Lte)
		case *Int64Filter_Gt:
			qb.AddCondition("local_peer_id_unique_key", ">", filter.Gt)
		case *Int64Filter_Gte:
			qb.AddCondition("local_peer_id_unique_key", ">=", filter.Gte)
		case *Int64Filter_Between:
			qb.AddBetweenCondition("local_peer_id_unique_key", filter.Between.Min, filter.Between.Max.GetValue())
		case *Int64Filter_In:
			if len(filter.In.Values) > 0 {
				qb.AddInCondition("local_peer_id_unique_key", Int64SliceToInterface(filter.In.Values))
			}
		case *Int64Filter_NotIn:
			if len(filter.NotIn.Values) > 0 {
				qb.AddNotInCondition("local_peer_id_unique_key", Int64SliceToInterface(filter.NotIn.Values))
			}
		default:
			// Unsupported filter type
		}
	...
	if req.OrderBy != "" {
		validFields := []string{"updated_date_time", "event_date_time", "topic_layer", "topic_fork_digest_value", "topic_name", "topic_encoding", "local_peer_id_unique_key", "meta_client_name", "meta_client_version", "meta_client_implementation", "meta_client_os", "meta_client_ip", "meta_client_geo_city", "meta_client_geo_country", "meta_client_geo_country_code", "meta_client_geo_continent_code", "meta_client_geo_longitude", "meta_client_geo_latitude", "meta_client_geo_autonomous_system_number", "meta_client_geo_autonomous_system_organization", "meta_network_name"}
		...
		orderByClause = " ORDER BY meta_network_name" + ", event_date_time" + ", meta_client_name" + ", local_peer_id_unique_key" + ", topic_fork_digest_value" + ", topic_name"
	}
	...
	return BuildParameterizedQuery("libp2p_leave", columns, qb, orderByClause, limit, offset, options...)
}

// BuildGetLibp2PLeaveQuery constructs a parameterized SQL query from a GetLibp2PLeaveRequest
func BuildGetLibp2PLeaveQuery(req *GetLibp2PLeaveRequest, options ...QueryOption) (SQLQuery, error) {
	...
	return BuildParameterizedQuery("libp2p_leave", columns, qb, orderByClause, 1, 0, options...)
}
