...
// SQL query builder for libp2p_reject_message
...
// BuildListLibp2PRejectMessageQuery constructs a parameterized SQL query from a ListLibp2PRejectMessageRequest
func BuildListLibp2PRejectMessageQuery(req *ListLibp2PRejectMessageRequest, options ...QueryOption) (SQLQuery, error) {
	...
	// Add filter for column: local_delivery
	if req.LocalDelivery != nil {
		switch filter := req.LocalDelivery.Filter.(type) {
		case *BoolFilter_Eq:
			qb.AddCondition("local_delivery", "=", filter.Eq)
		case *BoolFilter_Ne:
			qb.AddCondition("local_delivery", "!=", filter.Ne)
		default:
			// Unsupported filter type
		}
	}
	...
	// Add filter for column: reason
	if req.Reason != nil {
		switch filter := req.Reason.Filter.(type) {
		case *StringFilter_Eq:
			qb.AddCondition("reason", "=", filter.Eq)
		case *StringFilter_Ne:
			qb.AddCondition("reason", "!=", filter.Ne)
		case *StringFilter_Contains:
			qb.AddLikeCondition("reason", "%" + filter.Contains + "%")
		case *StringFilter_StartsWith:
			qb.AddLikeCondition("reason", filter.StartsWith + "%")
		case *StringFilter_EndsWith:
			qb.AddLikeCondition("reason", "%" + filter.EndsWith)
		case *StringFilter_Like:
			qb.AddLikeCondition("reason", filter.Like)
		case *StringFilter_NotLike:
			qb.AddNotLikeCondition("reason", filter.NotLike)
		case *StringFilter_In:
			if len(filter.In.Values) > 0 {
				qb.AddInCondition("reason", StringSliceToInterface(filter.In.Values))
			}
		case *StringFilter_NotIn:
			if len(filter.NotIn.Values) > 0 {
				qb.AddNotInCondition("reason", StringSliceToInterface(filter.NotIn.Values))
			}
		default:
			// Unsupported filter type
		}
	...
	if req.OrderBy != "" {
		...
		orderByClause = " ORDER BY meta_network_name" + ", event_date_time" + ", meta_client_name" + ", peer_id_unique_key" + ", topic_fork_digest_value" + ", topic_name" + ", message_id" + ", seq_number"
	}

	// Build column list
	columns := []string{"toUnixTimestamp(`updated_date_time`) AS `updated_date_time`", "toUnixTimestamp64Micro(`event_date_time`) AS `event_date_time`", "topic_layer", "topic_fork_digest_value", "topic_name", "topic_encoding", "seq_number", "local_delivery", "peer_id_unique_key", "message_id", "message_size", "reason", "meta_client_name", "meta_client_version", "meta_client_implementation", "meta_client_os", "meta_client_ip", "meta_client_geo_city", "meta_client_geo_country", "meta_client_geo_country_code", "meta_client_geo_continent_code", "meta_client_geo_longitude", "meta_client_geo_latitude", "meta_client_geo_autonomous_system_number", "meta_client_geo_autonomous_system_organization", "meta_network_name"}

	return BuildParameterizedQuery("libp2p_reject_message", columns, qb, orderByClause, limit, offset, options...)
}

// BuildGetLibp2PRejectMessageQuery constructs a parameterized SQL query from a GetLibp2PRejectMessageRequest
func BuildGetLibp2PRejectMessageQuery(req *GetLibp2PRejectMessageRequest, options ...QueryOption) (SQLQuery, error) {
	...
	return BuildParameterizedQuery("libp2p_reject_message", columns, qb, orderByClause, 1, 0, options...)
}
