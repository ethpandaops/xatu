...
// SQL query builder for libp2p_rpc_meta_control_ihave
...
// BuildListLibp2PRpcMetaControlIhaveQuery constructs a parameterized SQL query from a ListLibp2PRpcMetaControlIhaveRequest
func BuildListLibp2PRpcMetaControlIhaveQuery(req *ListLibp2PRpcMetaControlIhaveRequest, options ...QueryOption) (SQLQuery, error) {
	...
	// Add filter for column: message_index
	if req.MessageIndex != nil {
		switch filter := req.MessageIndex.Filter.(type) {
		case *Int32Filter_Eq:
			qb.AddCondition("message_index", "=", filter.Eq)
		case *Int32Filter_Ne:
			qb.AddCondition("message_index", "!=", filter.Ne)
		case *Int32Filter_Lt:
			qb.AddCondition("message_index", "<", filter.Lt)
		case *Int32Filter_Lte:
			qb.AddCondition("message_index", "<=", filter.Lte)
		case *Int32Filter_Gt:
			qb.AddCondition("message_index", ">", filter.Gt)
		case *Int32Filter_Gte:
			qb.AddCondition("message_index", ">=", filter.Gte)
		case *Int32Filter_Between:
			qb.AddBetweenCondition("message_index", filter.Between.Min, filter.Between.Max.GetValue())
		case *Int32Filter_In:
			if len(filter.In.Values) > 0 {
				qb.AddInCondition("message_index", Int32SliceToInterface(filter.In.Values))
			}
		case *Int32Filter_NotIn:
			if len(filter.NotIn.Values) > 0 {
				qb.AddNotInCondition("message_index", Int32SliceToInterface(filter.NotIn.Values))
			}
		default:
			// Unsupported filter type
		}
	...
	if req.OrderBy != "" {
		validFields := []string{"unique_key", "updated_date_time", "event_date_time", "rpc_meta_unique_key", "message_index", "control_index", "topic_layer", "topic_fork_digest_value", "topic_name", "topic_encoding", "message_id", "peer_id_unique_key", "meta_client_name", "meta_client_version", "meta_client_implementation", "meta_client_os", "meta_client_ip", "meta_client_geo_city", "meta_client_geo_country", "meta_client_geo_country_code", "meta_client_geo_continent_code", "meta_client_geo_longitude", "meta_client_geo_latitude", "meta_client_geo_autonomous_system_number", "meta_client_geo_autonomous_system_organization", "meta_network_name"}
		...
		orderByClause = " ORDER BY meta_network_name" + ", event_date_time" + ", unique_key" + ", control_index" + ", message_index" + ", meta_client_name"
	}

	// Build column list
	columns := []string{"unique_key", "toUnixTimestamp(`updated_date_time`) AS `updated_date_time`", "toUnixTimestamp64Micro(`event_date_time`) AS `event_date_time`", "rpc_meta_unique_key", "message_index", "control_index", "topic_layer", "topic_fork_digest_value", "topic_name", "topic_encoding", "message_id", "peer_id_unique_key", "meta_client_name", "meta_client_version", "meta_client_implementation", "meta_client_os", "meta_client_ip", "meta_client_geo_city", "meta_client_geo_country", "meta_client_geo_country_code", "meta_client_geo_continent_code", "meta_client_geo_longitude", "meta_client_geo_latitude", "meta_client_geo_autonomous_system_number", "meta_client_geo_autonomous_system_organization", "meta_network_name"}

	return BuildParameterizedQuery("libp2p_rpc_meta_control_ihave", columns, qb, orderByClause, limit, offset, options...)
}

// BuildGetLibp2PRpcMetaControlIhaveQuery constructs a parameterized SQL query from a GetLibp2PRpcMetaControlIhaveRequest
func BuildGetLibp2PRpcMetaControlIhaveQuery(req *GetLibp2PRpcMetaControlIhaveRequest, options ...QueryOption) (SQLQuery, error) {
	...
	return BuildParameterizedQuery("libp2p_rpc_meta_control_ihave", columns, qb, orderByClause, 1, 0, options...)
}
