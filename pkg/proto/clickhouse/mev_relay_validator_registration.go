...
// SQL query builder for mev_relay_validator_registration
...
// BuildListMevRelayValidatorRegistrationQuery constructs a parameterized SQL query from a ListMevRelayValidatorRegistrationRequest
func BuildListMevRelayValidatorRegistrationQuery(req *ListMevRelayValidatorRegistrationRequest, options ...QueryOption) (SQLQuery, error) {
	...
	// Add filter for column: fee_recipient
	if req.FeeRecipient != nil {
		switch filter := req.FeeRecipient.Filter.(type) {
		case *StringFilter_Eq:
			qb.AddCondition("fee_recipient", "=", filter.Eq)
		case *StringFilter_Ne:
			qb.AddCondition("fee_recipient", "!=", filter.Ne)
		case *StringFilter_Contains:
			qb.AddLikeCondition("fee_recipient", "%" + filter.Contains + "%")
		case *StringFilter_StartsWith:
			qb.AddLikeCondition("fee_recipient", filter.StartsWith + "%")
		case *StringFilter_EndsWith:
			qb.AddLikeCondition("fee_recipient", "%" + filter.EndsWith)
		case *StringFilter_Like:
			qb.AddLikeCondition("fee_recipient", filter.Like)
		case *StringFilter_NotLike:
			qb.AddNotLikeCondition("fee_recipient", filter.NotLike)
		case *StringFilter_In:
			if len(filter.In.Values) > 0 {
				qb.AddInCondition("fee_recipient", StringSliceToInterface(filter.In.Values))
			}
		case *StringFilter_NotIn:
			if len(filter.NotIn.Values) > 0 {
				qb.AddNotInCondition("fee_recipient", StringSliceToInterface(filter.NotIn.Values))
			}
		default:
			// Unsupported filter type
		}
	...
	if req.OrderBy != "" {
		...
		orderByClause = " ORDER BY meta_network_name" + ", event_date_time" + ", meta_client_name" + ", relay_name" + ", validator_index" + ", timestamp"
	}

	// Build column list
	columns := []string{"toUnixTimestamp(`updated_date_time`) AS `updated_date_time`", "toUnixTimestamp64Micro(`event_date_time`) AS `event_date_time`", "timestamp", "relay_name", "validator_index", "gas_limit", "fee_recipient", "slot", "toUnixTimestamp(`slot_start_date_time`) AS `slot_start_date_time`", "epoch", "toUnixTimestamp(`epoch_start_date_time`) AS `epoch_start_date_time`", "wallclock_slot", "toUnixTimestamp(`wallclock_slot_start_date_time`) AS `wallclock_slot_start_date_time`", "wallclock_epoch", "toUnixTimestamp(`wallclock_epoch_start_date_time`) AS `wallclock_epoch_start_date_time`", "meta_client_name", "meta_client_version", "meta_client_implementation", "meta_client_os", "meta_client_ip", "meta_client_geo_city", "meta_client_geo_country", "meta_client_geo_country_code", "meta_client_geo_continent_code", "meta_client_geo_longitude", "meta_client_geo_latitude", "meta_client_geo_autonomous_system_number", "meta_client_geo_autonomous_system_organization", "meta_network_name"}

	return BuildParameterizedQuery("mev_relay_validator_registration", columns, qb, orderByClause, limit, offset, options...)
}

// BuildGetMevRelayValidatorRegistrationQuery constructs a parameterized SQL query from a GetMevRelayValidatorRegistrationRequest
func BuildGetMevRelayValidatorRegistrationQuery(req *GetMevRelayValidatorRegistrationRequest, options ...QueryOption) (SQLQuery, error) {
	...
	return BuildParameterizedQuery("mev_relay_validator_registration", columns, qb, orderByClause, 1, 0, options...)
}
