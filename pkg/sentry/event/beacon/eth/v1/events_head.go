...
func NewEventsHead(log observability.ContextualLogger, event *eth2v1.HeadEvent, now time.Time, beacon *ethereum.BeaconNode, duplicateCache *ttlcache.Cache[string, time.Time], clientMeta *xatu.ClientMeta) *EventsHead {
	return &EventsHead{
		log:            log.WithField("event", "BEACON_API_ETH_V1_EVENTS_HEAD_V2"),
		now:            now,
		event:          event,
		beacon:         beacon,
...
func (e *EventsHead) Decorate(ctx context.Context) (*xatu.DecoratedEvent, error) {
	...
	if err != nil {
		e.log.WithError(err).WithContext(ctx).Error("Failed to get extra head data")
	} else {
		decoratedEvent.Meta.Client.AdditionalData = &xatu.ClientMeta_EthV1EventsHeadV2{
			EthV1EventsHeadV2: additionalData,
		}
	}
...
func (e *EventsHead) ShouldIgnore(ctx context.Context) (bool, error) {
	...
	if retrieved {
		e.log.WithFields(logrus.Fields{
			...
			"slot":                  e.event.Slot,
		}).WithContext(ctx).Debug("Duplicate head event received")
...
func (e *EventsHead) getAdditionalData(_ context.Context) (*xatu.ClientMeta_AdditionalEthV1EventsHeadV2Data, error) {
